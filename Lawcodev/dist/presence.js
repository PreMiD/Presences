...
owa.on("UpdateData", () => {
    let doc = document.location, path = doc.pathname, route = path.split("/"), browsingStamp = Math.floor(Date.now() / 1000), presenceData = {
        largeImageKey: "lawco",
    ...
    if (path === "/") {
        ...
        presenceData.state = `Artículo: ${document.querySelector('h1').innerHTML.length > 15 ? document.querySelector('h1').innerHTML.slice(0, 15) : document.querySelector('h1').innerHTML}...`;
        presenceData.startTimestamp = browsingStamp;
    }
        ...
        presenceData.state = `Con: ${document.querySelector(".list-group-item").childNodes[5].childNodes[0].textContent}`;
        presenceData.startTimestamp = browsingStamp;
    }
        ...
        try {
            ...
            presenceData.state = `${document.querySelectorAll(".title")[0].childNodes[1].innerHTML}`;
            presenceData.startTimestamp = browsingStamp;
        } catch {
