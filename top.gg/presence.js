//ty to eggsy
//seems to be required as everyone uses it
...
presence.on("UpdateData", () => __awaiter(this, void 0, void 0, function* () {
    ...
    var presenceData = {};
    presenceData.largeImageKey = "dblregular";
    ...
    if(window.location.pathname.startsWith("/moderation"))
    {
        ...
        {
            var personalquota = document.getElementsByClassName("quotaindiv")[0].textContent;
            var botamount = personalquota.substring(personalquota.indexOf('reviewed') + 9, personalquota.indexOf('/'))
    ...
    else if(window.location.pathname.startsWith("/bot/"))
    {
        ...
        else if(window.location.pathname.endsWith("/vote"))
        {
            ...
            presenceData.state = document.querySelector("#totalContent > div > p").textContent.trim();
        }
        ...
        {
            presenceData.details = "Viewing a Discord bot: " + document.querySelector("#bot-info > p.is-flex > span").textContent.trim();
            presenceData.largeImageKey = "dblstaff";
        ...
        else
        {
            ...
            presenceData.state = document.querySelector("#bot-info > p.is-flex > span").textContent.trim();
    ...
    else if(window.location.pathname.startsWith("/list/"))
    {
        ...
        presenceData.state = document.querySelector("#botlistitle").textContent.split("-")[0].trim();
    }
    ...
    else if(window.location.pathname.startsWith("/user/") || window.location.pathname == "/me")
    {
    ...
    else if(window.location.pathname.startsWith("/servers"))
    {
        ...
        if(window.location.pathname.startsWith("/servers/list/") || window.location.pathname.startsWith("/servers/tag/"))
        {
        ...
        else if(document.querySelector("#bot-info > p.is-flex.nameAndVoteThing > span"))
        {
            ...
            presenceData.state = document.querySelector("#bot-info > p.is-flex.nameAndVoteThing > span").textContent.trim();
        }
