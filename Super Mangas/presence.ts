...
presence.on("UpdateData", async () => {
	...
	if (document.title.includes("Top Conteúdo")) {
		...
		(host == "www.supermangas.site" && path.startsWith("/manga/")) ||
		(path.startsWith("/manhwa/") && path.replace("/manga/", "")) ||
		(path.startsWith("/manhua/") && path.replace("/manga/", ""))
	) {
		if (document.title.includes("Capítulo", "")) {
			mangaPage = document.querySelector(
				"#corpo > div.conteudoBox.box_suport > div.capBox > div.capPageContent > div.capMenu.capMenuFixedTop > select.capListPage > option.capituloPage.active"
			);
			mangaName = document.querySelector(
				"#corpo > div:nth-child(3) > div.videoSidebar > div.capaCategory > div > h3 > a"
			);
			mangaChapter = document.querySelector(
				"#corpo > div.conteudoBox.box_suport > div.capBox.focus > div.capPageContent > div.capMenu.capMenuFixedTop.fixed > select.capList > option:nth-child(1)"
			);
			...
			presenceData.state =
				document.querySelector(
					"#corpo > div:nth-child(1) > div:nth-child(2) > div.boxBarraInfo > h1"
				).innerText +
				" - " +
				document.querySelector(
					"#corpo > div:nth-child(1) > div:nth-child(2) > div.boxAnime > ul > div > li:nth-child(1) > span"
				).innerText +
				" Capítulos";
		...
		let rank = document.querySelector(
			"#corpo > div:nth-child(4) > div.postHomeVideoBox > div.menu_box_action > p:nth-child(2) > a:nth-child(1)"
		);
		if (rank) {
