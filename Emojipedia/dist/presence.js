...
presence.on("UpdateData", () => {
    ...
    if (document.URL === "https://emojipedia.org/" || document.URL === "https://emojipedia.org") {
        presenceData.details = "Staring at the main page";
    } else if (document.location.href.includes("https://blog.emojipedia.org")) {
        if (document.URL === "https://blog.emojipedia.org/" || document.URL === "https://blog.emojipedia.org") {
            presenceData.details = "Skimming through the blog homepage";
        } else {
            presenceData.details = "Reading a blog post...";
            presenceData.state = document.getElementsByClassName("post-full-title")[0].innerText;
        }
        ...
        for (n = 0; n < categoryURLs.length; n++) {
            if (document.URL.includes(categoryURLs[n])) isCategory = true, bypass = true;
        }

        if (bypass !== true) {
            for (n = 0; n < eventURLs.length; n++) {
                if (document.URL.includes(eventURLs[n])) isEvent = true, bypass = true;
            }
        }
        ...
        if (isCategory === true) {
            ...
            if (document.getElementsByTagName("h1").length >= 1) {
                ...
                if (emojiRegex.test(document.getElementsByTagName("h1")[0].innerText) === true) {
