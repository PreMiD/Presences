				...
				case 1:
					...
					presences = presenceFolders.map(function(pF) {
						var metadata = JSON.parse(
								fs_1.readFileSync(pF + "/dist/metadata.json", "utf-8")
							),
							presenceJs = fs_1.readFileSync(pF + "/dist/presence.js", "utf-8");
						var resJson = {
							name: metadata.service,
							url:
								"https://api.premid.app/v2/presences/" +
								encodeURI(metadata.service) +
								"/",
							metadata: metadata,
							presenceJs: presenceJs
						};
						if (metadata.iframe)
							resJson.iframeJs = fs_1.readFileSync(
								pF + "/dist/iframe.js",
								"utf-8"
							);
						return resJson;
					});
					(newPresences = presences.filter(function(p) {
						return !dbPresences.some(function(dP) {
							return dP.name === p.name;
						});
					})),
						(deletedPresences = dbPresences.filter(function(dP) {
							return !presences.some(function(p) {
								return p.name === dP.name;
							});
						})),
						(outdatedPresences = dbPresences
							.filter(function(p) {
								return presences.find(function(dp) {
									return (
										p.name === dp.name &&
										dp.metadata.version !== p.metadata.version
									);
								});
							})
							.map(function(dP) {
								return presences.find(function(p) {
									return p.name === dP.name;
								});
							}));
					(dP = []), (oP = []);
					...
					Promise.all(__spreadArrays([nP], dP, oP)).then(function() {
