		...
		if (PageRoute.length == 0) {
			...
			if (username) {
				if (PageRoute[2] === "comment") {
					presenceData.state = `${username.innerText} ❯ ${locales.user.comment}`;
				} else if (PageRoute[2] === "following") {
					presenceData.state = `${username.innerText} ❯ ${locales.user.friends.default}`;
				} else if (PageRoute[2] === "mutual-friends") {
					presenceData.state = `${username.innerText} ❯ ${locales.user.friends.mutual}`;
				} else {
					presenceData.state = username.innerText;
				}
			}
