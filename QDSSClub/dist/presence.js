...
presence.on("UpdateData", () => {
  ...
  if (document.location.pathname == "/") {
    if (document.location.href.startsWith("https://www.qdssclub.com/?page=")) {
      ...
      (data.details = "Nella homepage"),
        (data.state =
          ...
          hpforumpagenumber.replace("https://www.qdssclub.com/?page=", ""));
      data.startTimestamp = browsingStamp;
      presence.setActivity(data);
    } else if (
      document.location.href.startsWith("https://www.qdssclub.com/?articoli=")
    ) {
      var hparticolipagenumber = document.location.href;
      (data.details = "Nella homepage"),
        (data.state =
          "Sfoglia gli articoli. Pag: " +
          hparticolipagenumber.replace(
    ...
    (data.details = "Nella homepage"), (data.startTimestamp = browsingStamp);
    presence.setActivity(data);
  } else if (document.location.pathname.startsWith("/faq")) {
    (data.details = "Nella scheda FAQ"), (data.startTimestamp = browsingStamp);
    presence.setActivity(data);
    ...
    document.location.href.startsWith("https://www.qdssclub.com/forums?page=")
  ) {
    var fforumpagenumber = document.location.href;
    (data.details = "Sfoglia il forum"),
      (data.state =
        "Pagina: " +
        fforumpagenumber.replace("https://www.qdssclub.com/forums?page=", "")),
      (data.startTimestamp = browsingStamp);
    presence.setActivity(data);
    ...
    document.location.href.endsWith("/forums/category/Conosciamoci/")
  ) {
    (data.details = "Sfoglia il forum"),
      (data.state = "Categoria: Conosciamoci Pag: 1"),
      (data.startTimestamp = browsingStamp);
    ...
    (data.details = "Navigando..."), (data.startTimestamp = browsingStamp);
    presence.setActivity(data);
  }
});
