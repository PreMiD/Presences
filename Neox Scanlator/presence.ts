...
presence.on("UpdateData", async () => {
	if (host == "neoxscan.com" && path == "/") {
		...
		let mgNam = document.querySelector(
			"body > div.wrapper > div > div:nth-child(1) > div > h2"
		);
		if (document.title.includes("- Capítulo", "")) {
			mangaPage = document.querySelector(
				"body > div.container-fluid > div:nth-child(1) > div > div > div > div > button > span.filter-option.pull-left"
			);
			...
			if (
				!document
					.querySelector(
						"body > div.container-fluid > div:nth-child(1) > div > div > div"
					)
					.getAttribute("style")
					.split(";")[1]
					.includes("none")
