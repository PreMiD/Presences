...
presence.on("UpdateData", () => {
  ...
  if(window.location.pathname === "/") {
    let size   = document.getElementById("stats-hud").innerHTML.split("|").length,
        ...
        mass   = document.getElementById("stats-hud").innerHTML.split("|")[0].replace("Score: ", ""),
        modepr = document.getElementsByClassName("is-active")[2].id,
        mode   = modepr.charAt(0).toUpperCase() + modepr.substring(1),
        region = document.getElementsByClassName("is-active")[1].id.replace("europe", "EU").replace("america", "NA").replace("asia", "AS"),
    ...
    if(size === 2) {
      ...
      presenceData.smallImageText = document.getElementById("stats-hud").innerHTML.replace("|","•")
    } 
    else if(size === 3) {
      presenceData.details = `${region} ${mode} • Mass:  ${mass}`;
      presenceData.state   = `Nickname: ${tag} ${nickname}`

      presenceData.smallImageText = document.getElementById("stats-hud").innerHTML.replace(hstats[0] + " |", "").replace("|","•");
    }
  };
