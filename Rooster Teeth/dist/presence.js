...
presence.on("UpdateData", () => {
    ...
    if(path.includes("/live/rt-tv")){
        try{
            ...
            presenceData.details+=" "+document.querySelector(".livestream-card.livestream-schedule-item-fade-enter-done").querySelector(".livestream-title").innerText
        }catch(e){}
        ...
        try{
            ...
            if(document.querySelector(".video-details__heading")){
                presenceData.details=document.querySelector(".video-details__title").innerText
                presenceData.state=document.querySelector(".video-details__show").innerText
            }else{
        ...
        try{
            for(var scheduleDay of document.getElementsByClassName("schedule-day")){
                ...
                if(position.top < window.innerHeight && position.bottom >= 0) {
                    presenceData.state=scheduleDay.querySelector(".schedule-day__heading").innerText.toLowerCase()
                    presenceData.state=presenceData.state.substr(0,1).toUpperCase()+presenceData.state.substr(1)
                    break
                }
            }
        }catch(e){
        ...
        try{
            presenceData.state=document.querySelector(".carousel-container").querySelector(".carousel-title").innerText.split("RECENT EPISODES FROM ")[1]
        }catch(e){}
    }else if(path.includes("/series")){
        ...
        if(urlParams.get("channel_id")){
            ...
            presenceData.state=document.querySelector(".dropdown-label").innerText.split("FILTER (")[1].replace(")","")
        }else{
        ...
        try{
            if(path.includes("explore")){
                ...
                presenceData.state=document.querySelector(".content-sidebar").querySelector(".banner-title").innerText
            }
        }catch(e){}
    ...
    if(video!=undefined){
        if(live){
            ...
            presenceData.smallImageText=video.paused ? "Live Playback paused" : "Live"
        }else{
            presenceData.smallImageKey= video.paused ? "pause" : "play"
            presenceData.smallImageText=video.paused ? "Playback paused" : "Playing back"
            if(!video.paused){
                presenceData.endTimestamp=Math.floor(Date.now() / 1000) - Math.floor(video.currentTime) + Math.floor(video.duration);
            }else{
