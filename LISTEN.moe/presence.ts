...
function getArtists() {
    ...
    if (elemt != undefined) {
        for (let i = 0; i < elemt.length; i++) {artists.push(elemt[i].textContent.replace(/\s+/g,' ').trim())};
        artist = artists.join(' ');
...
presence.on("UpdateData", async () => {
    ...
    if (playback) {
        ...
        track = (document.querySelectorAll('input.search')[1] as HTMLInputElement).value;
