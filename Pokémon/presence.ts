...
presence.on("UpdateData", async () => {
	...
	if (document.location.host == "www.pokemon.com") {
		if (
			...
			let search = <HTMLInputElement>document.querySelector("#searchInput");
		...
		if (document.location.pathname.includes("/topic")) {
			...
			let input = <HTMLInputElement>(
				document.querySelector("#elMainSearchInput")
			);
		...
		if (document.location.pathname.includes("/news/")) {
			...
			let search = <HTMLInputElement>(
				document.querySelector("#word.form-control")
			);
			if (
		...
		if (document.location.pathname.includes("/main")) {
			...
			if (document.location.pathname.includes("/category")) {
				...
				(<HTMLLinkElement>(
					document.querySelector("#header-top-menu > li.active > a")
				)).href.includes("/event")
			) {
				...
				let input = <HTMLInputElement>document.querySelector("#search_text");
		...
		if (document.location.pathname.includes("/news/")) {
			...
			if (document.querySelector("#main > h1 > img") !== null) {
				...
				presenceData.state = (<HTMLImageElement>(
					document.querySelector("#main > h1 > img")
				)).alt;
			} else {
				presenceData.details = "Pokémon Movies";
		...
		if (document.location.pathname.includes("/anime")) {
			...
			presenceData.smallImageKey = "pokeball";
			presenceData.smallImageText = "Pokédex";
			let search = <HTMLInputElement>document.querySelector("#search_input");
