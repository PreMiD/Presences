...
presence.on('UpdateData', () => {
    ...
    if (Path.startsWith('/learn')) {
        ...
        presenceData.state = `${Path.replace('/learn/','').length < 1 ? 'Main Section' : `${Path.replace('/learn/','').toUpperCase()} Section`}`
    } else if (Path == '/') {
        switch (document.querySelector('body > div.flexrow.photopea > div:nth-child(1) > div.flexrow > div.panelblock.mainblock > div > div.panelhead').children.length) {
            ...
            default:
                ...
                presenceData.details = `Editing a ${Details.currentFile.name.split('.').length > 1 ? Details.currentFile.name.split('.')[Details.currentFile.name.split('.').length - 1] : Details.currentFile.name.split('.')[1]} file :`
                presenceData.state = Details.currentFile.name
        }
    } else if (Path.startsWith('/api')) {
        presenceData.details = `Consulting the api docs :`
        presenceData.state = `${Path.replace('/api/','').length < 1 ? 'Main Section' : `${Path.replace('/api/','').toUpperCase()} Section`}`
    } else if (Path.startsWith('/tuts')) {
        presenceData.details = `Consulting the tutorials :`
        presenceData.state = `${Path.replace('/tuts/','').length < 1 ? 'Main Section' : `${document.querySelector('#post-396 > h1').textContent}`}`
    } else {
