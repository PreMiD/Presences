...
presence.on("UpdateData", async () => {
    ...
    if (host == "shiftdelete.net") {
        const title = document.querySelector("body > div.wrapper > section > div > div.left.harber > h1"),
            author = document.querySelector("body > div.wrapper > section > div > div.left.harber > div.twtCommnets > div > div.desc > em"),
            time = document.querySelector("body > div.wrapper > section > div > div.left.harber > div.need_to_be_rendered") || document.querySelector("body > div.wrapper > section > div > div.left.harber > div:nth-child(4)");

        if (title && author && time && title.textContent != "" && author.textContent != "" && time.textContent != "") {
            ...
            presence.setActivity({
                ...
                state: searchingFor && searchingFor.textContent ? searchingFor.textContent.replace(/"/g, "") : "Belirsiz",
        ...
        if (page.includes("/members/") && user && user.textContent != "") {
            ...
            const newTitle = document.querySelector(".js-titleInput") as HTMLInputElement;
            ...
            presence.setActivity({
                ...
                state: forumTitle && forumTitle.textContent ? forumTitle.textContent : "Belirsiz",
                startTimestamp: Math.floor(Date.now() / 1000)
            });
        } else if (page.includes("/threads/")) {
            const title = document.querySelector("#top > div.p-body > div > div.p-body-header > div.p-title > h1"),
                author = document.querySelector("#js-XFUniqueId1"),
                time = document.querySelector("#top > div.p-body > div > div.p-body-header > div.p-description > ul > li:nth-child(2) > a > time");

            presence.setActivity({
                largeImageKey: "sd-logo",
                details: title && title.textContent != "" ? title.textContent : "Belirsiz Gönderi",
                state: `Yazar: ${author && author.textContent != "" ? author.textContent : "Belirsiz Gönderi Sahibi"} ${time && time.textContent != "" ? "(" + time.textContent + ")" : ""}`,
                smallImageKey: "forum",
