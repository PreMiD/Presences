...
iframe.on("UpdateData", async () => {
	...
	if (document.querySelector("#vstr > div.jw-wrapper.jw-reset > div.jw-media.jw-reset > video") !== null) {
		anime = <HTMLVideoElement>document.querySelector("#vstr > div.jw-wrapper.jw-reset > div.jw-media.jw-reset > video");
		if (anime != undefined && !isNaN(anime.duration)) {
			iframe.send({
				iframe_video: {
				iFrameVideo: true,
	...
	if (document.querySelector("#player > div.jw-media.jw-reset > video") !== null) {
		anime = <HTMLVideoElement>document.querySelector("#player > div.jw-media.jw-reset > video");
		if (anime != undefined && !isNaN(anime.duration)) {
			iframe.send({
				iframe_video: {
				iFrameVideo: true,
	...
	if (document.querySelector("div.html5-video-container > video") !== null) {
		anime = <HTMLVideoElement>document.querySelector("div.html5-video-container > video");
		if (anime != undefined && !isNaN(anime.duration)) {
			iframe.send({
				iframe_video: {
				iFrameVideo: true,
	...
	if (document.querySelector("#vplayer").getElementsByTagName("video")[0] !== null) {
		anime = <HTMLVideoElement>document.querySelector("#vplayer").getElementsByTagName("video")[0];
		if (anime != undefined && !isNaN(anime.duration)) {
			iframe.send({
				iframe_video: {
				iFrameVideo: true,
