...
presence.on("UpdateData", async () => {
	...
	if (document.location.pathname == "/") {
		...
		if (
			document.querySelector(
				"div.menu_filter_box2.box_content > ul > p > a.buttonLink.active"
			) === null
		) {
			data.details = name;
		...
		(data.details = "Página do Hunter"),
			(data.state =
				"Checando: " +
				document.querySelector(
					"#corpo > div:nth-child(1) > div.box_content > div > select > option:nth-child(1)"
				).innerText);
		data.startTimestamp = browsingStamp;
		presence.setActivity(data);
	...
	// ------------------------------ Perfil de hentai anime ------------------------------ //
		...
		if (
			document.querySelector(
				"#corpo > div > div.conteudoBox.js_videoBox > div.boxSubTitulo > h2"
			) === null
		) {
			...
			(data.details = name),
				(data.state = document.querySelector(
					"#corpo > div > div:nth-child(5) > div.videoSidebar > div.boxMenuEps > div:nth-child(1) > div.epsBoxSobre > a"
				).innerText);
			data.startTimestamp = browsingStamp;
			presence.setActivity(data);
		}
	...
	// ------------------------------ Perfil de manga hentai ------------------------------ //
		...
		if (
			document.querySelector(
				"#corpo > div.conteudoBox.box_suport > div.boxSubTitulo > h2"
			) === null
		) {
