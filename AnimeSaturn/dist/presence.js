        ...
        if (videoEpisode == null) {
          if (videoName != null) {
            ...
            (data.smallImageKey = paused ? "pause" : "play"),
              ...
              (data.details = "Guardando: " + videoName),
              (data.state = paused
                ? "OAV ~ " + videoEpisode + "｜In pausa"
                : "OAV ~ " + videoEpisode + "｜In riproduzione"),
              (data.startTimestamp = paused ? "" : timestamps[0]),
              (data.endTimestamp = paused ? "" : timestamps[1]),
              presence.setActivity(data);
          }
...
function setCookie(cookieName, cookieValue) {
  ...
  document.cookie =
    "pmd_" + cookieName + "=" + cookieValue + "_" + cookieName + "pmd; path=/";
}
function getCookie(cookieName) {
  var cookie = document.cookie
    .split("pmd_" + cookieName + "=")[1]
    .split("_" + cookieName + "pmd")[0];
