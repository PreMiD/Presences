  ...
  // ----------- barra de opções topo ----------- //
    ...
    (data.details = "Lista de mangás"),
      (data.state =
        "Ordenada por: " +
        (
          document.querySelector("#menu-titulos > li.active > a > span") as
          HTMLElement
        ).innerText);
    ...
    (data.details = "Lista de Categorias"),
      (data.state =
        "Ordenada por: " +
        (
          document.querySelector("#menu-categorias > li.active > a > span") as
          HTMLElement
        ).innerText);
    data.startTimestamp = browsingStamp;
    presence.setActivity(data);
    ...
    (data.details = "Lista de Grupos"),
      (data.state =
        "Ordenada por: " +
        (
          document.querySelector("#menu-grupos > li.active > a > span") as
          HTMLElement
        ).innerText);
    data.startTimestamp = browsingStamp;
    presence.setActivity(data);
