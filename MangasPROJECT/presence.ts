...
presence.on("UpdateData", async () => {
    ...
    if (document.location.pathname == ("/")) {
	  ...
	  // ----------- barra de opções topo ----------- //
		...
		data.state = "Ordenada por: " + document.querySelector("#menu-categorias > li.active > a > span").innerText;
		data.startTimestamp = browsingStamp;
		presence.setActivity(data);
	}
	else if (document.location.pathname.startsWith("/grupos")) {
		data.details = "Lista de Grupos",
		data.state = "Ordenada por: " + document.querySelector("#menu-grupos > li.active > a > span").innerText;
		data.startTimestamp = browsingStamp;
		presence.setActivity(data);
	}
	  ...
	  // ----------- Categorias (Categoria selecionada) e página da scan ----------- //
		...
		data.details = "Lista de Mangás",
		data.state = "Ordenada por: " + document.querySelector("head > title").innerText.replace("Mangás:", "");
		data.startTimestamp = browsingStamp;
		presence.setActivity(data);
	}
	else if (document.location.pathname.startsWith("/scanlator")) {
		data.details = "Scan " + document.querySelector("#wraper > div > div.content-wraper.scan-data > div > ul > li > div.series-info.touchcarousel > span.series-title").innerText;
