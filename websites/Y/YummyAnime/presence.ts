...
presence.on(
  ...
  (data: {
    ...
    playback = data.iframeData.dur !== null ? true : false;

    if (playback) {
      ({ iFrameVideo } = data.iframeData);
      currentTime = data.iframeData.currTime;
      duration = data.iframeData.dur;
      ({ paused } = data.iframeData);
    }
