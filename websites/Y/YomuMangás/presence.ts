	...
	if (privacy) {
		...
		if (manga && ((isAdult && adult) || !isAdult) && !(chapter && isReader)) {
			presenceData.buttons = [
				{
					label: "Ver Mangá",
					url: document.querySelector<HTMLAnchorElement>("#premid-manga-url")
						?.href,
				},
			];
		}
		if (manga && ((isAdult && adult) || !isAdult) && chapter && isReader) {
			presenceData.buttons = [
				...
				{
					label: "Ver Capítulo",
					url: document.querySelector<HTMLAnchorElement>("#premid-chapter-url")
						?.href,
				},
			];
		}
		...
		presenceData.buttons = [
			{
				label: "Ver Perfil",
				url: document.querySelector<HTMLAnchorElement>("#premid-user-url")
					?.href,
			},
		];
		if (images && user) {
