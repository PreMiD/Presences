...
async function checkDomain(): Promise<DomainCheckState> {
  const result: DomainCheckState = { invalid: true, validDomains: null },
    ...
    cookieName = "PMD_BLACKANIME_VALID_DOMAINS";
...
presence.on("UpdateData", async () => {
  if (!isDomainChecked) {
    ...
    await checkDomain().then((result) => {
      ...
      if (isClone) {
        presence.error(
          `The following blackanime domain is a clone therefore not supported by this extension. The valid domains are:\n${result.validDomains.join(
            "\n"
          )}`
        );
      } else {
        presence.success(
          "The following blackanime domain is supported by this extension."
        );
      }
    });
