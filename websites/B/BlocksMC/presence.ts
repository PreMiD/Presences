...
presence.on('UpdateData', async() => {
    ...
    if (path === '/friends') {
        ...
        let friends: String = 'Fetching...';

        for (let index = 0; index < elements.length; index++) {
            const element = elements.item(index);
            if (element.className === 'badge badge-success badge-pill') friends = element.innerText;
        }
        ...
        const name: String =
            document.getElementsByClassName('profile-header').item(0)
        ...
        const guild: String = 
            document.getElementsByClassName('sub-header').item(0)
                .getElementsByTagName('h1').item(1)
                .innerText.split(' Guild')[0];
        
        const members: String = 
            document.getElementsByClassName('table table-left').item(0)
        ...
        const page: String = pages[document.location.pathname];
