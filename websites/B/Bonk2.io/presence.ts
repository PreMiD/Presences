...
presence.on('UpdateData', async () => {
  ...
  if (element) {
    ...
    data.state = document.querySelector('#pretty_top_name').textContent + ' - ' + document.querySelector('#pretty_top_level').textContent;
    switch (element.id) {
      ...
      case 'newbonklobby':
        ...
        data.details = `In a lobby - ${lastGameMode} (${playerCount} player${playerCount === 1 ? '' : 's'})`;
