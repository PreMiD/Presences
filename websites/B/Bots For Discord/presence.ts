...
presence.on("UpdateData", async () => {
    ...
    if (window.location.hostname == 'botsfordiscord.com') {
        ...
        if (document.querySelector('head > title').innerHTML.split("|")[0].trim().startsWith('Error')) {
        ...
        } else if (window.location.pathname.toLowerCase() == '/me' || window.location.pathname.startsWith("/user/")) {
            presenceData.details = "Viewing a profile:";
            presenceData.state = document.querySelectorAll("div.container #highlight")[1].innerHTML.trim();
        } else if (window.location.pathname.toLowerCase() == '/me/favorites') {
            presenceData.state = document.querySelectorAll("div.navbar-item a")[8].innerHTML.split("#")[0].trim() + " favorite bots";
            ...
            presenceData.state = document.querySelector('div.container h1').innerHTML.split('</i>')[1].trim();
