...
if (path.hostname == "blockdit.com" || path.hostname.includes ("www.")) {
  ...
  if (document.location.pathname == "/") {
    ...
    if (path.href.includes("bookmark")) {
      ...
      const page = document.querySelector("head > title").textContent ?? "Unknow page";
  ...
  } else if (path.pathname.includes ("explore") || path.pathname.includes("all")) {
    presenceData.startTimestamp = browsingStamp;
    ...
    const user = document.querySelector("head > title").textContent ?? "Unknow username";
    presenceData.startTimestamp = browsingStamp;
    presenceData.details = "Viewing on profile";
    ...
    const searchres = document.querySelector("#__next > div > div.container.flex-grow-1.d-flex.flex-column > div > div.jsx-3959159148.col-12.col-md-8.col-lg-6.col-xl-5.px-0 > div > div:nth-child(2) > div > form > div.jsx-1849148460.d-flex.align-items-center > label > input").getAttribute("value") ?? "Unknow search";
    presenceData.startTimestamp = browsingStamp;
