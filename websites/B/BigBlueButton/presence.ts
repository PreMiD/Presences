    ...
    if (inCall) {
        ...
        document.querySelectorAll("section").forEach(el => {
            if (el.className.startsWith("actionsbar")) {
                userState =
                    el.querySelector('i.icon-bbb-desktop') ? "screen"
                        : el.querySelector('i.icon-bbb-video') ? "video"
                            : el.querySelector('i.icon-bbb-desktop_off') ? "presentation"
...
presence.on("UpdateData", async () => {
    const userStateText =
        userState == "screen" ? "Sharing screen..."
            : userState == "presentation" ? "Presenting..."
                : userState == "video" ? "Video call"
