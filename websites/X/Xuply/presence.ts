...
presence.on("UpdateData", () => {
  ...
  if (page.toLowerCase() === "/") {
    ...
    presenceData.state = document.location.search ? document.querySelector("body > main > div > div > div > div > nav > ol > li:nth-child(3) > a").textContent : "Tüm Kodlar";
    presenceData.startTimestamp = browsingStamp;
    ...
    presenceData.state =
      document.querySelector("body > main > div > div > div > div > div > h2").textContent +
      " " +
      "adlı kullanıcının profilini görüntülüyor!";
    presenceData.startTimestamp = browsingStamp;
  } else if(page.startsWith('/code')) {
	presenceData.details = "Bir kod görüntülüyor:";
	presenceData.state = document.querySelector("body > main > div > div > div > div > div > h2").textContent + " " + "adlı kodu görüntülüyor."
    presenceData.startTimestamp = browsingStamp;
  }
