...
presence.on("UpdateData", async () => {
  const presenceData: PresenceData = {
    ...
    porcent: HTMLElement = document.querySelector("#settings-section > div > ul > li:nth-child(1) > div > div"),
    currentChapTitle: HTMLElement = document.querySelector("body > div:nth-child(2) > main > section.landing.novel-single > div.novel-head.pt-3 > div > div > div:nth-child(2) > div > h1 > a"),
  ...
  if (path == "/" || !path) {
    if (document.body.contains(trad_orig)) {
      ...
      presenceData.state = "Só olhando... Que estranho!"; /* Juist lookin, how strange! */
      presenceData.startTimestamp = time;
    }
    ...
    presenceData.state = notice.innerText;
      ...
      if (document.body.contains(favoritar)){
        ...
        presenceData.details = "Lendo " + currentChapTitle.innerText; /* reading */
        presenceData.state = "Em " + porcent.innerText + " do " +  ChapNumber.innerText; /*in... of...*/
        presenceData.startTimestamp = time;
      }
