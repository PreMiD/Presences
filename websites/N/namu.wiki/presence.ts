...
presence.on("UpdateData", async () => {
  ...
  else if (action === "RecentChanges")
    page = changesMapping[getParam(params, 'logtype')];

  /* View Discuss Thread */
  else if (action === "thread" || action === "edit_request")
    page = document.querySelector(`h1 > a`).textContent; // H1 Tag Only one
  ...
  if (details !== undefined) {
    const members = document.querySelectorAll(`#app > div > div > nav > ul[class=r] > li > div > div > div`);
