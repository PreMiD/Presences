...
presence.on('UpdateData', async () => {
  ...
  itemName = document.querySelector('[itemprop="name"]') ? document.querySelector('[itemprop="name"]').textContent : null;
  ...
  if (location.hostname !== 'www.newgrounds.com') {
    ...
    if (document.location.pathname.startsWith('/movies')) {
      ...
      if (document.location.pathname.startsWith('/audio/listen')) {
        const artist = document.querySelector('.item-details-main').textContent.trim();
        ...
        if (document.querySelector('#audio-listen-play[style="display: none;"]')) {
      ...
      if (document.location.pathname.startsWith('/art/view')) {
        ...
        if (await presence.getSetting('itemname'))
          presenceData.state = itemName.length < 2 ? 'Art: ' + itemName : itemName;
      }
      ...
      if (document.location.pathname.startsWith('/portal/view')) {
        ...
        switch (metaType.getAttribute('content')) {
          case 'movie':
            ...
            if (await presence.getSetting('itemname'))
              presenceData.state = itemName.length < 2 ? 'Movie: ' + itemName : itemName;
            ...
            if (document.querySelector('.ng-video-player')) {
              ...
              if (document.querySelector('[data-action="play"][style="display: none;"]')) {
                if (await presence.getSetting('timestamp')) {
                  const timestamps = getTimestamps(
                    document.querySelector('[data-value="current-time"]').textContent,
                    document.querySelector('[data-value="total-time"]').textContent
                  );
                  presenceData.startTimestamp = timestamps[0];
                  presenceData.endTimestamp = timestamps[1];
                }
          ...
          case 'game':
            ...
            if (await presence.getSetting('itemname'))
              presenceData.state = itemName.length < 2 ? 'Game: ' + itemName : itemName;
      ...
      if (document.location.pathname.startsWith('/bbs/search/author/')) {
        const userName = document.querySelector('.pod-head > .search').textContent.split('"')[1];
        ...
        if (await presence.getSetting('topicname'))
          presenceData.state = itemName.length < 2 ? 'Topic: ' + itemName : itemName;
      }
      ...
      if (document.location.pathname.split('/')[2]) {
        ...
        if (await presence.getSetting('itemname'))
          presenceData.state = itemName.length < 2 ? 'Collection: ' + itemName : itemName;
      }
      ...
      if (document.location.pathname.startsWith('/playlists/view')) {
        ...
        if (await presence.getSetting('itemname'))
          presenceData.state = itemName.length < 2 ? 'Playlist: ' + itemName : itemName;
      }
      ...
      presenceData.details = 'Searching for ' + document.querySelector('.current').textContent;
  ...
  if (document.location.pathname.startsWith('/skins.html')) {
    ...
    if (document.querySelector(addPremadeSkinSelector)) {
      const skinCategory = document.querySelector('#smpskins[style*='block'] #smpcatid');
      presenceData.state = skinCategory ? `Adding a ${skinCategory.textContent} skin` : 'Adding a skin';
