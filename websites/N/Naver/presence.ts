...
presence.on("UpdateData", async () => {
    ...
    getImageOrTimestamp = (video: HTMLVideoElement, type: "starts" | "ends" | "imageKey" | "imageText") => {
    ...
    data.presence = {
      ...
      "/": {
        ...
        data: {
          details: {
            setTo: `Translating from: ${document.querySelector("#ddSourceLanguageButton > span")?.textContent}`
          },
          state: {
            setTo: `To: ${document.querySelector("#ddTargetLanguageButton > span")?.textContent}`
          },
          smallImageKey: {
            setTo: `${data.service.toLowerCase()}_language`
          }
