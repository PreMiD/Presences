...
presence.on('UpdateData', async () => {
  const presenceData: PresenceData = {
    largeImageKey: 'https://cdn.rcd.gg/PreMiD/websites/N/Neovel/assets/logo.jpg',
  ...
  const [privacy, time, showCover] = await Promise.all([
    presence.getSetting<boolean>('privacy'),
    presence.getSetting<boolean>('time'),
    presence.getSetting<boolean>('cover'),
  ])
  const bookCoverPath = 'https://neovel.io/V2/book/image?bookId=%bookId%&oldApp=false&imageExtension=1'
  ...
  switch (pathnameArray[1]) {
    ...
    case 'book':
      if (document.location.hostname.includes('neopload')) {
        ...
        presenceData.details = presenceDataSlide.details = 'Reading a book page'
  ...
  if (!showCover) {
    presenceData.largeImageKey = presenceDataSlide.largeImageKey = 'https://cdn.rcd.gg/PreMiD/websites/N/Neovel/assets/logo.jpg'
  }
