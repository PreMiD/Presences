  ...
  presence.on("UpdateData", async () => {
    ...
    if (window.location.hostname == "forum.kryptonia.fr") {
      ...
      if(window.location.pathname.startsWith("/threads/")) {
        data.state = "📝" + (document.querySelector("#top > div.p-body-header > div > div > div.p-title > h1 > span.label-append") ? "" : " ") + document.querySelector("#top > div.p-body-header > div > div > div.p-title > h1").textContent
        .replace("Accepté(e)", "")
        .replace("Refusé(e)", "")
        .replace("Résolu(e)", "")
        .replace("Important", "");
