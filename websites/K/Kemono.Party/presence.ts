...
switch (document.location.hostname) {
    case "kemono.party": {
        ...
        default:if (location.pathname.includes("/user/")) { 
            if (location.pathname.includes("/post/")) {
                presenceData.details = `${document.querySelector("#page > header > div.post__info > h1 > span:nth-child(1)").textContent}`;
                presenceData.state = `${document.querySelector("a[class='post__user-name']").textContent.replace(/\s+/g, "")}`;
            } else {
                presenceData.details = "Checking out:"; 
                presenceData.state = `${document.querySelector("#user-header__info-top > a > span:nth-child(2)").textContent}`;
			presenceData.largeImageKey = `${(<HTMLImageElement>document.querySelector("#main > section > header > a > picture > img")).src}`;
			}
		}
	}
        break;
