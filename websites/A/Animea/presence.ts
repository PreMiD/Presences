...
presence.on('UpdateData', async () => {
  const presenceData: PresenceData = {
    largeImageKey: 'https://cdn.rcd.gg/PreMiD/websites/A/Animea/assets/logo.png',
    startTimestamp: browsingTimestamp,
  }
  const pathnameArray = document.location.pathname.split('/')
  const [time, showCover, showButtons] = await Promise.all([
  ...
  switch (pathnameArray[1]) {
    ...
    case 'discovery': {
      ...
      const queryText = document
        ...
        ?.trim()
      if (queryText !== 'Tout')
        presenceData.state = `Type : ${queryText}`
      break
    }
    case 'series':
    case 'movies': {
      ...
      const queryText = document
        ...
        ?.trim()
      if (queryText !== 'Tout')
        presenceData.state = `Catégorie : ${queryText}`
      break
    }
    case 'categories':
  ...
  if (!showCover) {
    presenceData.largeImageKey = 'https://cdn.rcd.gg/PreMiD/websites/A/Animea/assets/logo.png'
  }
  if (!showButtons)
