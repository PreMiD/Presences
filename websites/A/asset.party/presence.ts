...
presence.on("UpdateData", async () => {
	...
	if (pathname === "/")
		...
		presenceData.state = `🔑${KeyDetails[0].innerHTML.match(/\d+/)[0]} 🧍 ${KeyDetails[2].innerHTML.match(/\d+/)[0]} 👀 ${KeyDetails[2].innerHTML.match(/\d+/)[0]} `;
		presenceData.smallImageKey = "key";
		presenceData.smallImageText = "torture.";
	} else if (pathname === "/api-history/") 
		presenceData.details = "Viewing API History";
	else if (document.querySelectorAll(".nav-thumb").length !== 0 && document.querySelectorAll(".page-header-block").length !== 0) {
		
		const Details = document.querySelectorAll(".page-header-block");
		if (Details.length !== 0 && document.querySelectorAll(".nav-thumb").length !== 0) {
