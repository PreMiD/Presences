...
presence.on(
    "iFrameData",
    (data: { video: boolean; duration: number; currentTime: number; paused: boolean }) => {
    video = data;
    }
);
...
presence.on("UpdateData", async () => {
    ...
    if (
        document.querySelector(".infoan2") !== null && document.querySelector("#iframemain") !== null && video.video) {
        ...
        presenceData.state = document.querySelector("#main > div.now2 > div").textContent.split(" - ")[1];
        ...
        presenceData.state = document.querySelector(".lisbg").textContent.split(": ")[1];
        presenceData.smallImageKey = "reading";
    } else if (document.location.pathname.includes("/categories")) {
