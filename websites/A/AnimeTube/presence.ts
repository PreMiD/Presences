...
presence.on("UpdateData", async () => {
  ...
  if (document.location.pathname === "/") {
    ...
    [, , presenceData.state] = document.title.split("›").map((s) => s.trim());
    ...
    const [, userName] = document.title.split("Profil von ");

    presenceData.details = "| Benutzerinfo";
    presenceData.state = `Profil von ${userName}`;
    ...
    const [, , animeName, episode] = document.title
      .split("›")
      .map((s) => s.trim());
