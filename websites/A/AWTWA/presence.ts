...
presence.on("UpdateData", async () => {
	...
	if (pathname === "/") {
		...
		const animeName = document.querySelector("h1")?.textContent;
		const poster = document.querySelector("div[role='img']")?.getAttribute("style").split("url(")[1].split(")")[0].replace(/['"]/g, "");
		const episode = (document.getElementById("episode") as HTMLSelectElement).value;
		const season = (document.getElementById("season") as HTMLSelectElement).value;

		presenceData.details = presenceStrings.onPage + animeName;
		presenceData.state = "Saison " + season + " - Épisode " + episode;
