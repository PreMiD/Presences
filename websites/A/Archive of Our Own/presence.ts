  ...
  presence.getSetting<string>("tag"), ]);
    ...
    if (document.location.hostname == "archiveofourown.org") {
      ...
      if (document.location.pathname === "/")
        ...
        if (tag) {
        presenceData.details = `Browsing tag : ${document.querySelector(".heading > .tag").textContent} `;
        presenceData.state = `Page  : ${document.querySelector("h2.heading").textContent}`;
        ...
        if (work) {
        presenceData.details = `Reading : ${document.querySelector("h2").textContent}`;
        presenceData.state = `${document.querySelector("div.chapter > h3").textContent}`;
        ...
        presenceData.details = `Viewing Series : ${document.querySelector("h2.heading").textContent}`;
        presenceData.state = `By ${document.querySelector("a[rel=author]").textContent}`;
        presenceData.buttons = [
          {
            label: "View",
            url: window.location.origin + window.location.pathname,
