const presence = new Presence({
  ...
  getSeconds = (minutes: string, seconds: string): number => {
    const parsedMinutes = parseInt(minutes),
      minutesToSeconds = Math.floor(parsedMinutes * 60),
      result = minutesToSeconds + Number(seconds);

    return result;
  },
  truncateBefore = (str: string, pattern: string): string => {
    return str.slice(str.indexOf(pattern) + pattern.length);
  },
  truncateAfter = (str: string, pattern: string): string => {
