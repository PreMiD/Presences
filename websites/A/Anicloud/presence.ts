...
presence.on("iFrameData", async (data: {currentTime: number, timeEnd: number, paused: boolean, played:boolean}) => {
...
presence.on("UpdateData", async () => {
  const page = document.location.pathname,
    ...
    germanepisodename = document.querySelector(
      "h2"
    ) as HTMLElement,

    genre = document.querySelector(
      "#wrapper > div.container.marginBottom > div.seriesListNavigation > strong") as HTMLElement,
    ...
    forumname = document.querySelector(
      "h5"
    ) as HTMLElement,
