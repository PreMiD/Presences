...
const title = document.querySelector('.panel-heading')?.textContent ?? 'ไม่ทราบชื่อ'
const titlemovie = document.querySelector('.panel-heading')?.textContent ?? 'ไม่ทราบชื่อเรื่อง'
const ep = document.querySelector('.panel-heading')?.textContent ?? 'ไม่ทราบชื่อตอน'
const playvdo = document.querySelector(
...
presence.on('UpdateData', async () => {
  ...
  const presenceData: PresenceData = {
    largeImageKey: 'https://cdn.rcd.gg/PreMiD/websites/A/Anime-D/assets/logo.jpg',
  ...
  if (document.location.pathname === '/') {
    ...
    if (playvdo.includes('ตอนที่')) {
      ...
      titlemovie?.includes('The Movie')
      || titlemovie?.includes('เดอะมูวี่')
      || titlemovie?.includes('เดอะมูฟวี่')
      || titlemovie?.includes('มูฟวี่')
    ) {
      const info = titlemovie?.split('The Movie') || titlemovie?.split('พากย์ไทย')
      Movie = info!.pop()
