...
interface AnimeResponse {
  ...
  rating: number;
  genres: string[];
  seasons: { seasonNumber: number; title: string; episodes: { episodeNumber: number; title: string }[] }[];
}
...
async function getUserInfo(username: string): Promise<UserResponse["data"] | null> {
  try {
    ...
    return data.success ? { ...data.data, avatar: data.data.avatar || "logo" } : null;
  } catch {
    return null;
  }
}
...
presence.on("UpdateData", async () => {
  ...
  if (path === "/") {
    ...
    presenceData.largeImageKey = userInfo?.avatar;
    presenceData.startTimestamp = time;
    presenceData.buttons = [{ label: "Profile Bak", url: `${baseUrl}/u/${username}` }];
