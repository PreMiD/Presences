class AppleTV extends Presence {
  ...
  getVideo(){
    return document.querySelector("apple-tv-plus-player")
                    .shadowRoot.querySelector("amp-video-player-internal")
                    .shadowRoot.querySelector("amp-video-player")
                    .shadowRoot.querySelector("#apple-music-video-player") as HTMLVideoElement;
  }

  getTitle(eyebrow = false){
    if (this.isWatching()){
      const title = document.querySelector("apple-tv-plus-player")
                            .shadowRoot.querySelector("amp-video-player-internal")
                            .shadowRoot.querySelector("div.info__eyebrow")?.textContent;

      if(title || eyebrow) return title;
      else return document.querySelector("apple-tv-plus-player")
                          .shadowRoot.querySelector("amp-video-player-internal")
    ...
    data.presence = {
      ...
      "/person/([a-zA-Z0-9-]+)": {
        setPresenceData(){
          ...
          presenceData.state = document.querySelector("div.person-header__bio > h1")?.textContent;
        }
      },
      "/settings": {
        setPresenceData(){
