...
presence.on("UpdateData", () => {
  ...
  if (url.includes("post")) {
    ...
    if (url.includes("submit")) {
      ...
      const problemName = document.querySelector("#content > h2 > a").textContent.trim();
      ...
      const problemName = document.querySelectorAll(".tabs > h2 > a")[document.querySelectorAll(".tabs > h2 > a").length - 1].textContent.trim(),
      ...
      presenceData.buttons.push({label: "View Submissions", url: problemSubmissionsURL});
      ...
      presenceData.state = problemName;
      presenceData.largeImageKey = "submission_list";
      presenceData.buttons.push({label: "View Best Submissions", url: problemBestSubmissionsURL});
      ...
      presenceData.buttons.push({label: "View Editorial", url: problemEditorialURL});
      ...
      const problemName = document.querySelector(".problem-title > h2").textContent.trim(),
      ...
      if (problemPoints === "1") 
        ...
        presenceData.state = `${problemName} (${problemPoints} points)`;
    ...
    if (url.includes("user")) {
      ...
      const submissionHeader = document.querySelector(".tabs > h2").textContent.trim();

      presenceData.details = "Viewing submissions by user:";
      
      if (submissionHeader === "All my submissions") {

        const user = document.querySelector("#user-links > ul > li > a > span > span > b").textContent.trim(),
          userSubmissionsURL = `https://dmoj.ca/submissions/user/${user}`,
          userURL = `https://dmoj.ca/user/${user}`;

        presenceData.state = user;
        presenceData.largeImageKey = "submission_list";
        presenceData.buttons = [{label: "View Submissions", url: userSubmissionsURL}, {label: "View User", url: userURL}];
        
      } else {

        const user = document.querySelector(".tabs > h2 > a").textContent.trim(),
          userSubmissionsURL = `https://dmoj.ca/submissions/user/${user}`,
          userURL = `https://dmoj.ca/user/${user}`;
    ...
    const [,,organizationCode] = url,
      ...
      organizationName = document.querySelector("#content > h2").textContent.trim().replace(" Members", "");

    presenceData.state = organizationName;
    presenceData.buttons = [{label: "View Organization", url: organizationURL}, {label: "View Members", url: organizationMembersURL}];
    ...
    if (url.includes("solved")) {
      ...
      const [,userRank] = document.querySelectorAll(".user-sidebar > div")[2].textContent.trim().split("#");
      ...
      if (userHeader === "My account") {
        ...
        presenceData.largeImageKey = "submission_list";
        presenceData.buttons = [{label: "View User", url: userURL}, {label: "View Solved Problems", url: userSolvedProblemsURL}];
        
      } else {

        const [,user] = document.querySelector(".tabs > h2").textContent.trim().split(" "),
          userURL = `https://dmoj.ca/user/${user}`,
          userSolvedProblemsURL = `${userURL}/solved`;
    ...
    if (url.includes("stats")) {
      ...
      presenceData.buttons.push({label: "View Statistics", url: statisticsURL});
      ...
      presenceData.largeImageKey = "users";
      presenceData.buttons.push({label: "View Participation", url: participationURL});
