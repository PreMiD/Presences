                ...
                switch (pathSegments[1]) {
                    ...
                    case "quiz": {
                        ...
                        // every type except all type
                            ...
                            // doing quiz
                                ...
                                if (img) {
                                    const urlMatch = window.getComputedStyle(img).getPropertyValue("background-image").match(/url\(["']?([^"']*)["']?\)/);
                    ...
                    case "activity": {
                        ...
                        if (pathSegments[2] !== "" && !isNaN(Number(pathSegments[2]))) {
                            presenceData.state = titleText;
                            presenceData.largeImageKey = document.querySelectorAll("a.link img")[0].getAttribute("src");
                            presenceData.smallImageKey = Assets.Logo;
                        } else if (pathSegments[3] !== "" && !isNaN(Number(pathSegments[3]))) {
                            const texts = document.querySelector("h2.header");
                            presenceData.largeImageKey = document.querySelector("img.sharethumb").getAttribute("src");
                            presenceData.smallImageKey = Assets.Logo;
                    ...
                    case "studyabroad": {
                        ...
                        if (pathSegments[2] !== "" && !isNaN(Number(pathSegments[2]))) {
                            presenceData.state = titleText;
                            presenceData.largeImageKey = document.querySelectorAll(".image img")[0].getAttribute("src");
                            presenceData.smallImageKey = Assets.Logo;
            ...
            switch (pathSegments[1]) {
                ...
                case "article": {
                    const headerTexts = document.querySelectorAll(".special-intro")[0],
                        headerT = headerTexts ? headerTexts.querySelectorAll("h2")[0].textContent : "";
                    presenceData.smallImageKey = Assets.Logo;
                ...
                case "novel": {
                    ...
                    if (novelCoverElement) {
                        const urlMatch = window.getComputedStyle(novelCoverElement).getPropertyValue("background-image").match(/url\(["']?([^"']*)["']?\)/);
                        presenceData.largeImageKey = urlMatch ? urlMatch[1] : "";
                    }
