...
presence.on(
  'iFrameData',
  (data: unknown) => {
    stream = data as { duration: number, currentTime: number, paused: boolean }
  },
)
...
presence.on('UpdateData', async () => {
  ...
  const presenceData: PresenceData = {
    largeImageKey: 'https://cdn.rcd.gg/PreMiD/websites/D/Diziroll/assets/logo.png',
  }
  ...
  if (path.startsWith('/arsiv')) {
    ...
    if (!stream.paused) {
      [presenceData.startTimestamp, presenceData.endTimestamp] = presence.getTimestamps(
        stream.currentTime ? Math.floor(stream.currentTime) : 0,
        stream.duration ? Math.floor(stream.duration) : 0,
      )
    }
  }
  else {
