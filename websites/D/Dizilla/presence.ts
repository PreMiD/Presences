...
presence.on('UpdateData', async () => {
  ...
  if (path.startsWith('/dizi/')) {
    ...
    presenceData.state = document.title.slice(0, document.title.indexOf('arşivleri'))
      || 'Bilinmeyen Kanal'
  }
    ...
    if (query) {
      ...
      presenceData.state = query[0].toUpperCase() + query.slice(1).toLowerCase()
    ...
    presenceData.details = 'Bir sayfaya göz atıyor:'
    presenceData.state = pages[path] || pages[path.slice(0, -1)] || 'Bilinmeyen Sayfa'
  }
  else if (
    ...
    presenceData.details = `${showName.textContent
      .charAt(0)
