...
presence.on("UpdateData", async () => {
  const page : string = document.location.pathname,
        movieTitle = document.querySelector("#t1 > li > div.ssag > h1")?.textContent.split('|')[0].replace('izle', ''),
  ...
  if (document.querySelector("#t1 > li > div.ssag > h1")?.textContent && video?.currentTime && isVideoData) {
    ...
    if (!isNaN(timestamps[0]) && !isNaN(timestamps[1])) {
      ...
      if (movieTitle.includes('Sezon') && movieTitle.includes('Bölüm')) presenceData.details = 'Bir dizi izliyor:';
      ...
      if (movieTitle.includes('Sezon') && movieTitle.includes('Bölüm')) presenceData.details = 'Bir diziye göz atıyor:';
