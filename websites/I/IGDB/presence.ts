...
presence.on("UpdateData", async () => {
	...
	switch (pathname.split("/")[1]) {
				...
				default:
					if (pathname.split("/")[3] === "reviews")
						...
						presenceData.largeImageKey =
							document.querySelector<HTMLImageElement>(".mui-image-img").src;
		...
		case "genres":
			...
			} else presenceData.details = strings.browseGenres;
			break;
		case "platforms":
			if (pathTwo) {
				...
				presenceData.largeImageKey =
					document.querySelector<HTMLImageElement>(".logo_med").src;
			} else presenceData.details = strings.browsePlatforms;
			break;
		case "top-100":
		...
		case "themes":
			if (pathTwo) {
				...
				presenceData.state = document.title.slice(0, -5);
			} else presenceData.details = strings.browseThemes;
			break;
		case "collections":
			if (pathTwo) {
				presenceData.details = strings.viewCollection;
				presenceData.state = document.title;
			} else presenceData.details = strings.browseCollections;
			break;
		case "player_perspectives":
			if (pathTwo) {
				presenceData.details = strings.viewPlayerPerspectives;
				presenceData.state = document.title.slice(0, -5);
			} else presenceData.details = strings.browsePlayerPerspectives;
			break;
		case "franchises":
			if (pathTwo) {
				presenceData.details = strings.viewFranchises;
				presenceData.state = document.title;
			} else presenceData.details = strings.browseFranchises;
			break;
		case "categories":
			if (pathTwo) {
		...
		case "companies":
			if (pathTwo === "new") {
				...
				presenceData.largeImageKey =
					document.querySelector<HTMLImageElement>(".logo_med").src;
			} else presenceData.details = strings.browseCompanies;
			break;
		case "game_engines":
			if (pathTwo) {
				...
				presenceData.state = document.title;
				presenceData.largeImageKey =
					document.querySelector<HTMLImageElement>(".img-responsive").src;
			} else presenceData.details = strings.browseGameEngines;
			break;
		case "characters":
			if (pathTwo) {
				...
				presenceData.state = document.title;
				presenceData.largeImageKey =
					document.querySelector<HTMLImageElement>(".cover_big").src;
			} else presenceData.details = strings.browseCharacters;
			break;
		case "events":
			if (pathTwo) {
			...
			} else presenceData.details = strings.viewEvent;
			break;
		case "seach":
