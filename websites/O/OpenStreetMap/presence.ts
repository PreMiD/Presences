...
if (x && y) {
	fetch(
		...
		.then(res => {
			...
			[locationcity, locationstate, locationcountry] = [res.address.isolated_dwelling ?? res.address.hamlet ?? res.address.village ?? res.address.town ?? res.address.city, res.address.state, res.address.country];
		})
		.catch();
}
}, 5000);
...
} else if (document.location.hostname.startsWith("blog") && !document.location.hostname.startsWith("blogs")) {
...
} else if (document.location.pathname.includes("/search") && !document.location.pathname.includes("nominatim")) {
	presenceData.details = "Search results for:";
	presenceData.state = document.title.split("|")[0];
} else if (document.location.pathname.includes("/user/")) {
	presenceData.details = `On ${document.title.split("|")[0].trim()}'s profile:`;
