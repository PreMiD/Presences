...
presence.on("UpdateData", () => {
  ...
  if (path.endsWith("inbox")) {
    title = document.querySelector("#ReadingPaneContainerId > div > div > div > div._29NreFcQ3QoBPNO3rKXKB0 > div._3Yr_hO7j5doGUkhrRiP6uY > div:nth-child(1) > div > div._31eKqae41uP_KBAvjXjCLQ > div > div > div > div > div.ms-FocusZone.css-41 > div > div > input");
    if (!title) {
      presenceData.state = "Inbox";
    } else {
      presenceData.state = "New Email";
    }
  } else if (path.endsWith("junkemail")) {
    presenceData.state = "Junkemail";
  } else if (path.endsWith("drafts")) {
    presenceData.state = "Drafts";
  } else if (path.endsWith("sentitems")) {
    presenceData.state = "Sent Items";
  } else if (path.endsWith("deleteditems")) {
    presenceData.state = "Deleted Items";
  } else if (path.endsWith("archive")) {
    presenceData.state = "Archived Items";
    ...
    presenceData.state = "Personal Folder";
  }
