...
presence.on("UpdateData", async () => {
  ...
  if (document.location.pathname == "/") {
    ...
    presenceData.state = document.getElementsByClassName("ipsType_reset ipsPageHead_barText")[0].textContent;
    presenceData.startTimestamp = Math.floor(Date.now() / 1000);
    ...
    presenceData.state = document.getElementsByClassName("ipsType_break ipsContained")[0].textContent;
    presenceData.startTimestamp = Math.floor(Date.now() / 1000);
  } else if (document.location.pathname.startsWith("/search/")){
    delete presenceData.details;
    presenceData.details = "Searching...";
    presenceData.smallImageKey = "search";
    presenceData.state = "Looking" + document.getElementsByClassName("ipsType_reset ipsType_large")[0].textContent.split("results")[1];
    presenceData.startTimestamp = Math.floor(Date.now() / 1000);
  } else {
    delete presenceData.details;
    switch (document.location.pathname) {
