...
function getGenre(code: string) {
  ...
  if (firstCode === "E")
    ...
    else if (endCode.startsWith("02") && endCode !== "0207" && endCode !== "0208") return "POP";
...
presence.on("UpdateData", async () => {
  ...
  if (location.pathname === "/player/fPlayer") {
    ...
    else if (location.pathname.startsWith("/myMusic/purchase") || location.pathname === "/myMusic/oneClickDownload")
      presenceData.details += "(보관함)";
    else if (location.pathname.startsWith("/myMusic/gift") || location.pathname === "/myMusic/coupleList")
