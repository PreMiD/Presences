...
switch(window.location.hostname) {
	case "www.g2g.com": {
		...
		if(document.location.pathname === "/") {
			...
			presenceData.state = document.querySelector<HTMLHeadingElement>("title").textContent;
			presenceData.startTimestamp = browsingTimestamp;
			...
			presenceData.state = document.querySelector<HTMLBodyElement>("div.text-h4").textContent;
		} else if (document.location.pathname.includes("secure")) {
			const item:string = document.querySelector<HTMLBodyElement>("div.col-8.row").textContent;
