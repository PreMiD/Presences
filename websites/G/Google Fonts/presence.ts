...
presence.on("UpdateData", async () => {
    ...
    if(path === "/") {
        const categspan = document.querySelector("#main-content > gf-global-toolbar > div.global-toolbar__filters-area > gf-global-filters-row > div > div > gf-toolbar-category-filter > button > span.mat-button-wrapper"),
        categ = categspan.getElementsByClassName("gf-outlined-menu-button-content")[0].textContent;
        if (categ === " Categories ") {
            presenceData.state = "All categories";
        } else {
            if (categ.includes("+")) {
                let UrlResearch = (new URL(document.location.href)).searchParams,
        ...
        const fontName = document.querySelector("#main-content > gf-sticky-header > div > h1").textContent,
        fontMenu = document.getElementsByClassName("gf-nav__link--active")[0].textContent;
        ...
        if(document.getElementsByClassName("gmat-headline-1")[0]) {
            featuredFonts = document.getElementsByClassName("gmat-headline-1")[0].textContent;
        } else if (!document.getElementsByClassName("gmat-headline-1")[0]) {
            featuredFonts = document.getElementsByClassName("gmat-headline-4")[0].textContent;
        }
