...
presence.on('UpdateData', async () => {
  ...
  const showDomain = await presence.getSetting<boolean>('domain')
  ...
  let presenceData: PresenceData = {
    largeImageKey: 'https://cdn.rcd.gg/PreMiD/websites/G/Google%20Domains/assets/logo.jpg',
  ...
  if (path.includes('/m/registrar/')) {
    ...
    const tab = getElement('.dreg-ogb-menu-item-selected')
      || getElement('.partner-header')
      || getElement('.gb_bd')
    presenceData.state = domainName && showDomain ? `${domainName} (${tab})` : tab && `${tab} tab`
  }
  ...
  if (path.includes('/m/registrar/search/')) {
    ...
    presenceData.state = showSearch ? document.querySelector('input')?.value : undefined
  }
