...
presence.on("UpdateData", async() => {
    ...
    if(window.location.hostname == "gitlab.com"){
        if(document.location.pathname == "/" || document.location.pathname.includes("/dashboard/projects")){
        ...
        // document.querySelector("#dashboard_search")
            ...
            title = document.querySelector("body > div.layout-page.page-with-contextual-sidebar > div.content-wrapper > div.alert-wrapper > nav > div > div > ul > li:nth-child(2) > a > span");
            owner = document.querySelector("body > div.layout-page.page-with-contextual-sidebar > div.content-wrapper > div.alert-wrapper > nav > div > div > ul > li:nth-child(1) > a");
            if(title && owner){
                presenceData.startTimestamp = browsingStamp;
        ...
        title = document.querySelector("body > div.wrapper.gitlab-ee-page > div.image-title > div > div > h1");
        if(title){
            ...
            if(title){
                ...
                if(title){
                    ...
                    title = document.querySelector("body > div.wrapper.blog-landing > div.blog-landing-content > div.blog-hero > div > a > h2");
                    if(title){
                        presenceData.details = "Viewing " + title.textContent;
                        presenceData.startTimestamp = browsingStamp;
                    }
                    else if (title == null){
                        title = document.querySelector("body > div.blog.article > div.cover > div > div > h1");
                        if(title){
                            presenceData.details = "Viewing " + title.textContent;
                            presenceData.startTimestamp = browsingStamp;
                        }
                        else{
                            title = document.querySelector("body > div.reseller-hero.text-center > div > h1");
                            if(title){
                                presenceData.details = "Viewing " + title.textContent;
                                presenceData.startTimestamp = browsingStamp;
                            }
