...
presence.on(UpdateData, async () = {
  ...
  if (document.location.hostname == www.gog.com) {
    if(document.querySelector(body  div.wrapper.cf._prices-in-gbp._price-currency-symbol-before  div  divnth-child(3)  div)) {
      presenceData.details = Looking At;
      presenceData.state = Main Page;
    } else if (document.location.pathname.startsWith(game)) {
    presenceData.details = Looking At Game;
    if(!document.querySelector(body  div.layout.ng-scope  divnth-child(8)  div.product-actions.hide-when-content-is-expanded.ng-scope  div.product-actions-price.product-actions-price--discounted  span.product-actions-price__discount.ng-binding)) {
    presenceData.state = document.querySelector(body  div.layout.ng-scope  divnth-child(8)  div.productcard-basics.hide-when-content-is-expanded  h1).innerHTML;
    } else {
    presenceData.state = document.querySelector(body  div.layout.ng-scope  divnth-child(8)  div.productcard-basics.hide-when-content-is-expanded  h1).innerHTML + (+ document.querySelector(body  div.layout.ng-scope  divnth-child(8)  div.product-actions.hide-when-content-is-expanded.ng-scope  div.product-actions-price.product-actions-price--discounted  span.product-actions-price__discount.ng-binding).innerHTML +);
    } 
    ...
    if(showmoney) {
      presenceData.state = Total  + document.querySelector(body  div.universe  div.wrapper.cf  div  article  div  form  div.l-section.l-section--primary  section  div  div.form__row.form__row--summary.form__row--key-value  div._spinner._spinner--right-aligned.form__row-value  p.form__sheet-row.form__price.form_price--total  strongnth-child(2)  span  span).innerHTML;
    }
    ...
    if(document.location.pathname.startsWith(feed)) {
      ...
      presenceData.details = Looking At;
      presenceData.state = document.querySelector(body  div.layout  section.layout__header  main  div  div  a).innerHTML+'s Profile;
    } else if(document.location.pathname.startsWith(profileedit)) {
