const presence = new Presence({
  ...
  presence.on("UpdateData", async function () {
    ...
    if (document.location.hostname == "www.joyn.de" || document.location.hostname == "joyn.de") {
        if (document.querySelector('.lk71lm-0.htJLsh')) {
        ...
        else if ((urlpath[1] == '' || document.location.pathname.includes("/#home")) && urlpath[2] != '') {
            ...
            const video_startTime = Date.now(),
            video_endTime = Math.floor(video_startTime / 1000) - video.currentTime + video.duration;
            presenceData.details = document.querySelector(".metadataWrapper .metadataTitle").textContent;
