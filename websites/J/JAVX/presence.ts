...
presence.on("UpdateData", async () => {
  ...
  if (pathname === "/" && !search) {
    ...
    presenceData.details = keyword
      ...
      : (await strings).const presence = new Presence({
    clientId: "830476272978362408"
  }),
  strings = presence.getStrings({
    viewHome: "general.viewHome",
...
;
    ...
    presenceData.details = (await strings).viewAccount;
    presenceData.state = `${upperFirstLetter(role.toLowerCase())}: ${document.querySelector("h2").textContent.trim()}`;
  } else if (pathname.startsWith("/category")) {
    ...
    const 
      ...
      title = document
        .querySelector("#censorship > a")
        .textContent.trim() === "Porn" ? engname : id,
      actresses: string[] = [],
      studios: string[] = [];
