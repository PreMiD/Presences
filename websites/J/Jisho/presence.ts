	...
	if (document.location.hostname === "jisho.org") {
		if (document.location.pathname === '/') {
			...
			if (document.location.pathname.length < 8) {
				...
				presenceData.state = document.querySelector("#page_container > div > article.discussion_thread > div.first_comment > div.comment_body > div.discussion_title > h2").innerHTML;
			}
		}
		else if (document.location.pathname.startsWith("/users")) {
			presenceData.details = "Viewing a user:";
			presenceData.state = document.querySelector("#page_container > div > div.page > article > h2").innerHTML;
		}
		else if (document.location.pathname.startsWith("/sentences")) {
			presenceData.details = "Reading a sentence:";
			presenceData.state = document.querySelector("#page_container > div > div.page > article.sentences > li.sentence > div.sentence_content > div.english_sentence > span.english").innerHTML;
		}
