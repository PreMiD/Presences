  ...
  if (Routes[0] === "") {
    ...
    if (Queries.types && Queries.types.length === 1) {
      ...
      switch (Queries.types[0]) {
        case "10":
          data.state = `Ищет японское ранобэ`;
          break;
        case "11":
          data.state = `Ищет корейское ранобэ`;
          break;
        case "12":
          data.state = `Ищет китайское ранобэ`;
          break;
        case "13":
          data.state = `Ищет английское ранобэ`;
          break;
        case "14":
          data.state = `Ищет авторские`;
          break;
        case "9":
          data.state = `Ищет фанфики`;
          break;
        default:
          data.state = `Ищет ранобэ`;
          break;
      }
    ...
    if (Routes[1] === '') {
      ...
      switch (Queries.category) {
        case 'all': data.state = 'Все категории';
          break;
        case '1': data.state = 'Баги и проблемы';
          break;
        case '2': data.state = 'Предложения для сайта';
          break;
        case '3': data.state = 'Поиск тайтлов';
          break;
        case '4': data.state = 'Поиск кадров';
          break;
        case '5': data.state = 'Обсуждение Манги';
          break;
        case '6': data.state = 'Обсуждение Аниме';
          break;
        case '7': data.state = 'Обсуждение Ранобэ';
          break;
        case '8': data.state = 'Видеоигры';
          break;
        case '9': data.state = 'Переводчикам';
          break;
        case '10': data.state = 'Как переводить мангу';
          break;
        case '11': data.state = 'Как рисовать мангу';
          break;
        case '12': data.state = 'Общение';
          break;
        case '13': data.state = 'Другое';
          break;
      }
      ...
      if (Routes[2] && !Routes[3]) {
        ...
        const titleElement = document.querySelector(".discussion .discussion__title"),
        authorElement = document.querySelector(".discussion .discussion-creator__username");
        ...
        data.state = DiscussionTitle ? 'Редактирует тему: ' + DiscussionTitle : 'Редактирует тему';
      }
    }
    ...
    if (Routes[1] === "article") {
      ...
      switch(Queries.section) {
        case '1': data.state = 'Ранобэ';
          break;
        case '2': data.state = 'Общие вопросы';
          break;
        case '3': data.state = 'Профиль пользователя';
          break;
        case '4': data.state = 'Чтение ранобэ';
          break;
        case '5': data.state = 'Комментарии';
          break;
        case '6': data.state = 'Мини-чат';
          break;
        case '7': data.state = 'Решение проблем';
          break;
        case '8': data.state = 'Правила';
          break;
        case '9': data.state = 'Форум';
          break;
      }
    }
    ...
    switch(Queries.type) {
      case 'chapter': data.state = 'Главы';
        break;
      case 'comments': data.state = 'Комментарии';
        break;
      case 'message': data.state = 'Сообщения';
        break;
      case 'friend': data.state = 'Заявки в друзья';
        break;
      case 'other': data.state = 'Другое';
        break;
      case 'all': data.state = 'Все';
        break;
    }
    ...
    if (Routes[1] === "create") {
      ...
      if (!Routes[2]) {
        ...
        switch(Queries.section) {
          case 'info': data.state = "Редактирует информацию команды";
            break;
          case 'users': data.state = "Редактирует участников команды";
            break;
        }
      }
    ...
    if (!Routes[1]) {
      ...
      if (Routes[2] === 'rejected') {
        ...
        data.state = 'Модерация ранобэ';
      }
    } else if (Routes[1] === 'manga-edit') {
      data.state = 'Изменения ранобэ';
    ...
    if (Routes[1] === 'content') {
      ...
      switch(Queries.section) {
        case 'info': data.state = 'Информация';
          break;
        case 'site-settings': data.state = 'Настройки сайта';
          break;
        case 'notifications': data.state = 'Уведомления';
          break;
        case 'password': data.state = 'Безопасность';
          break;
      }
    } else {
      ...
      if (!Routes[2]) {
        // Main user page
        const size = document.querySelector(".bookmark-menu .menu__item.is-active .bookmark-menu__label"),
          title = document.querySelector(".bookmark-menu .menu__item.is-active .bookmark-menu__name");
  
        BookmarkSize = size && size.textContent;
        BookmarkType = title && title.textContent.charAt(0).toUpperCase() + title.textContent.slice(1);
    ...
    if (Routes[1] === "create") {
      ...
      if (title.value.length > 1) {
        ...
        data.state = "Имя ранобэ не задано";
      }
      ...
      if (Routes[2] === "edit") {
        ...
        switch(Queries.section) {
          case 'media-edit': data.state = 'Редактирование';
            break;
          case 'changes': data.state = 'Список изменений';
            break;
          case 'related-items': data.state = 'Связанные тайтлы';
            break;
        }
