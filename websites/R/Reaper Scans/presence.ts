...
presence.on("UpdateData", () => {
  ...
  if (
    ...
    if (/^\/$/.test(pathname)) data.details = "Viewing Home Page";
      ...
      // Counting comics
      comics = document.querySelectorAll(".page-listing-item .row .col-4")
        .length;
      data.details = "Viewing Comic List";
      data.state = `📋 ${comics.toString()} comics found`;
    } else if (/^\/all-series\/novels+\/?$/.test(pathname)) {
      // Counting novels
      comics = document.querySelectorAll(".page-listing-item .row .col-6")
        .length;
      ...
      const comicLink = (document.querySelector(
          "ol.breadcrumb li:nth-child(3) a"
        ) as HTMLAnchorElement).href,
        chapter = document.querySelector("ol.breadcrumb li:nth-child(4)")
          .textContent;
