...
presence.on("UpdateData", () => {
  ...
  if (Path == "/" || Path == "/~") {
    ...
    if (Path.replace("/@", "").split("/").filter((elm) => elm !== "").length === 1) {
    ...
    const language: string = (document.querySelector("#languageSelector-input") as HTMLInputElement).value;
    ...
    const repls: HTMLDivElement = document.querySelector(
     ...
     length: number = repls ? repls.children.length - (Path === "/repls" ? 2 : 1) : 0;
