...
presence.on("UpdateData", async () => {
  ...
  if (pathname === "/" || pathname === "/~")
    ...
    if (
      pathname.replace("/@", "").split("/").filter(elm => elm !== "").length ===
      1
    ) {
      ...
      let lang = document.querySelector<HTMLDivElement>(
        "div.language-icon-container"
      ).title;
      let replthing = await fetch(location.href, { credentials: "omit" });
      presenceData.details = `Editing a ${lang} repl`;
      if (replthing.status == 404) {
        presenceData.state = `Private Repl`;
      } else {
        presenceData.state = `${pathname
          .split("/")
          .filter(elm => elm !== "")[1]}${window.location.hash
    ...
    switch (pathname.replace("/community/", "")) {
      ...
      default:
        presenceData.state = `${postType
          .charAt(0)
          .toUpperCase()}${postType.slice(1)}${postElement
