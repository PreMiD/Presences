...
function formatText(message: string) {
	if (message.length > 127) message = `${message.substring(0, 124)}...`;
	return message;
}
...
presence.on("UpdateData", async () => {
	...
	if (pathname === "/") {
		...
		if (document.querySelector<HTMLDivElement>("#report"))
			...
			if (!privacyMode) presenceData.state = formatText(letterInput.value);
		} else if (
			document
			...
			if (!privacyMode)
				presenceData.state = formatText(messageContainer.textContent);
		} else {
