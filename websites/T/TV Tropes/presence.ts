...
presence.on("UpdateData", () => {
  const 
    [, pathCheck, mainPath, namespace] = const presence = new Presence({
    clientId: "864573021762224129"
  }),
  startTimestamp = Math.floor(Date.now() / 1e3),
  whitespaceRegex = /^\s*|\n/gm;
...
.split("/"),
  ...
  function mainWikiPathDetails(namespace: string) {
    if (namespace === "Main") {
      ...
      presenceData.details = "Viewing a page";
      presenceData.state = `${namespace === "Main"
            ? null
            : titleElement
                .querySelector("strong")
                .textContent.replace(/ \/ $/, "")} / ${pageText}`;
    }
  }
  ...
  if (pathCheck === "pmwiki") {
    switch (mainPath) {
      ...
      case "wysiwyg_source_editor.php": {
        const 
          searchParams = new URLSearchParams(const presence = new Presence({
    clientId: "864573021762224129"
  }),
  startTimestamp = Math.floor(Date.now() / 1e3),
  whitespaceRegex = /^\s*|\n/gm;
...
presence.on("UpdateData", () => {
  ...
  if (presenceData.details) presence.setActivity(presenceData);
          ...
          title = searchParams.get("title");
        presenceData.details = "Editing TV Trope Page";
        presenceData.state = `${searchParams.get("groupname")} / ${title}`;
        break;
      }
      case "createconversation.php": {
        presenceData.details = "Creating a Forum Post";
