...
if (document.location.pathname == "/") {
  ...
  const postTitle = (document.querySelector("#postcontent > h3")as HTMLElement).innerText;
  const userName = (document.querySelector("#user-info > div > h4 > a:nth-child(1)")as HTMLElement).innerText;
  const userHandle = (document.querySelector("#user-info > div > h4 > a:nth-child(2)")as HTMLElement).innerText;
   presenceData.startTimestamp = browsingStamp;
