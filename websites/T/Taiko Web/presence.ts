...
presence.on("UpdateData", async () => {
  ...
  if (canvas) {
    ...
    switch (const presence = new Presence({
    clientId: "858246998561783828"
  }),
  slideshow = presence.createSlideshow(),
  browsingTimestamp = Math.floor(Date.now() / 1000),
...
presence.on("UpdateData", async () => {
  ...
  } else if (document.querySelector<HTMLDivElement>("div#loading-don")) presenceData.details = "Game Loading ...";
  else if (document.querySelector<HTMLDivElement>("div.view")) presenceData.details = "Changing Game Settings";
