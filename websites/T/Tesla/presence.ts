...
presence.on("UpdateData", async function () {
  ...
  if(window.location.hostname === "www.tesla.com") {
    ...
    if(urlpath[urlpNum] === "category" && urlpath[num]) {
      ...
      presenceData.state = document.querySelector('h2.product-title.tds-text--h1-alt').textContent;
    ...
    } else if(urlpath[urlpNum] === "checkout" && urlpath[num] === "billing-shipping-info") {
      const costs = (set_showCheckout) ? ` (${document.querySelector('span.ordersummary__container__order__details__line__total>span.inline-value').textContent})` : "";
