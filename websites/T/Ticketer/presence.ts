...
const browsingUnix = Math.floor((Date.now() / 1000) | 0)

presence.on('UpdateData', async () => {
  const presenceData: PresenceData = {
    largeImageKey: 'https://cdn.rcd.gg/PreMiD/websites/T/Ticketer/assets/logo.png',
  }
  ...
  switch (pathname) {
    ...
    case `/dashboard/${document.URL.slice(34)}`: {
      let dashboardInfo: HTMLElement | null = null
      if (document.querySelector('.server-info > h3')) {
        const title = document.querySelector('.server-info > h3')
        const logo = document.querySelector('.server-info > img')
