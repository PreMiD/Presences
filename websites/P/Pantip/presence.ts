...
const club1 = document.querySelector(
...
presence.on('UpdateData', async () => {
  ...
  if (
    ...
    if (document.location.pathname === '/') {
      ...
      presenceData.state = document.querySelector('#search-text')?.getAttribute('value')
        ?? 'ไม่ทราบการค้นหา'
