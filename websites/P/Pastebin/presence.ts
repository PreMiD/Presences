...
presence.on("UpdateData", () => {
  ...
  const urlData = window.location.pathname.replace(/^\/([^/]*).*$/, "$1"); // Get everything after the / in the URL, for paste ID's
  ...
  if (urlData.length == 8) {
    ...
    const pasteName = element.getAttribute("title"); // Getting the title from the HTML tag
