...
presence.on("UpdateData", async () => {
  ...
  } else if (pathname.includes("/wish/tally") || pathname.includes("/wish/tally/")) 
    presenceData.details = "🌏 Global Wish Tally";
    
  else if (pathname.startsWith("/wish") || pathname.startsWith("/wish/")) {
    const ww = document.querySelector("#sapper > main > div > div.flex.flex-col.xl\\:flex-row > div.grid.gap-4.grid-cols-1.md\\:grid-cols-2.xl\\:grid-cols-3.max-w-screen-xl > div:nth-child(6) > div.bg-item.rounded-xl.p-4.flex.items-center.w-full.text-white"),
