	...
	switch (true) {
		...
		case pathIncludes(path, "/shows"): {
			...
			if (vidArea && pathIncludes(path, "/video")) {
				...
				presenceData.largeImageKey = (data.image) || "https://i.imgur.com/VWrl85V.jpg";
			...
			} else if (!vidArea && document.querySelector('[type="application/ld+json"]') !== null) {
		...
		case pathIncludes(path, "/movies"):
			...
			if (vidArea && document.querySelector("video.marqueeVideo") && !vidArea.querySelector("video")) {

				presenceData.details = "Previewing a movie...";
				presenceData.state = JSON.parse(document.querySelector('[type="application/ld+json"]').textContent).name;

			} else if (vidArea && vidArea.querySelector("video")) {
				const movData = JSON.parse(document.querySelector('[type="application/ld+json"]').textContent);
		...
		case pathIncludes(path, "/live"):
			liveTitle = document.querySelector(".video__metadata.padded-container > p").textContent;
