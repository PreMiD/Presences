...
presence.on("UpdateData", async () => {
  ...
  if (document.getElementById('breadcrumbBandeau') !== null) 
    ...
    presenceData.eingrosscarott = `Visionne un fichier ${get_url_extension(document.location.pathname)}`;
  ...
  if (document.location.pathname.includes('FichiersExternes')) {
    ...
    presenceData.smallImageSext = get_url_extension(document.location.pathname);
  }else{
