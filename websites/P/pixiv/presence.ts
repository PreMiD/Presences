...
presence.on("UpdateData", async () => {
  ...
  if (document.location.hostname.includes("www.pixiv.net")) {
    if (curPath.includes("/users")) {
      ...
      presenceData.state = document.querySelector("#root > div > div:nth-child(2) > div > div > div > div:nth-child(2) > div > div:nth-child(2) > div > h1").innerHTML;
    } else if (curPath.includes("/tags")) {
      presenceData.details = "Viewing tags:";
      presenceData.state = `${document.querySelector("#root > div > div:nth-child(2) > div > div:nth-child(2) > div:nth-child(2) > div > div > span").textContent}`;
      ...
      if (curPath.includes("/works"))
        presenceData.details = `Managing ${curPath.includes("/series") ? "Series" : "Artworks"}`;
      ...
      presenceData.state = document.querySelector<HTMLElement>("#stacc_center_title").innerText;
    } else if (curPath.includes("/fanbox"))
      presenceData.details = "Viewing fanbox";
    else if (curPath.includes("/event")) {
      const title = document.querySelector("#contents > div.pane.full.group > h1");
      if (title !== null) {
    ...
    } else if (curPath.includes("/setting_social_login.php") || curPath.includes("/linked_services")) {
      presenceData.details = "User settings";
      ...
      presenceData.details = `Submiting New ${typeResult == "manga" ? "Manga" : curPath.includes("novel") ? "Novel" : "Illustrations"}`;
      ...
      presenceData.smallImageKey = "search";
      presenceData.state = `${document.querySelector("#root > div:nth-child(2) > div:nth-child(2) > div > div > main > section > div > div > figcaption > div > div > h1").textContent} (${document.querySelector("#root > div:nth-child(2) > div:nth-child(2) > div > div > aside > section > h2 > div > div > a").textContent})`;
    } else if (curPath.includes("/novel/")) {
      const title = document.querySelector("#root > div > div:nth-child(2) > div > div > main > section > div > div:nth-child(2) > div:nth-child(2)"),
        author = document.querySelector("#root > div > div:nth-child(2) > div > div > aside > section > h2 > div > div > a");
      presenceData.details = title !== null ? "Viewing novel:" : "Browsing for novels...";
      if (title !== null) presenceData.state = `${title.textContent} (${author.textContent})`;
    ...
    if (curPath == "/" || curPath.includes("/public")) {
      ...
      const title = document.querySelector<HTMLElement>("#LiveSidebarLiveUser > div.name");
      ...
      presenceData.state = document.querySelector<HTMLElement>("#TagHeader > div > div.CarouselOverlay > div > div").innerText;
    }
  }
