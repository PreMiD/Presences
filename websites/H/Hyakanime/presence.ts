...
presence.on("UpdateData", async () => {
	...
	if (page.startsWith(routes.anime)) {
		presenceData.state = document.querySelector(".anime-title")?.textContent?.split("Adulte")[0];
	...
	if (page.startsWith(routes.user)) {
		...
		presenceData.state = document.querySelector(".user-banner__username")?.textContent;
		presenceData.largeImageKey = document.querySelector(".user-banner__picture")?.getAttribute("src") || Assets.Logo;
		presenceData.smallImageKey = Assets.Logo;
		presenceData.buttons = [{
