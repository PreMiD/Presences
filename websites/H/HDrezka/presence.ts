...
presence.on("UpdateData", async () => {
	...
	const currentType = contentType === "films" ? "фильм"
	...
	if (
		...
		if (document.location.pathname.match(/\/(films|series|cartoons)\/.+/)) {
			const contentTitle = document.querySelector(".b-post__title h1").textContent;
			const image = document.querySelector<HTMLImageElement>(".b-sidecover a img").src;
