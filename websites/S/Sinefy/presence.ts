...
presence.on('UpdateData', async () => {
  ...
  const presenceData: PresenceData = {
    largeImageKey: 'https://cdn.rcd.gg/PreMiD/websites/S/Sinefy/assets/logo.jpg',
    startTimestamp,
  }
  const settings = {
  ...
  if (page.includes('/izle/')) {
    ...
    if (episode)
      presenceData.state = episode.replace('izle', '')
    ...
    presenceData.state = document.querySelector('.bg-cover-faker h1.page-title')?.textContent
      ?? 'Bilinmeyen Kategori'
    ...
    presenceData.state = document.querySelector('.generic-box h2.title-secondary a')
    ...
    presenceData.state = document.querySelector('.story-detail .story-header h1.title-primary')
