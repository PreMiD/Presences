...
presence.on("UpdateData", async () => {
    ...
    function subjectCondition(subject: string): String{
            ...
            if(path == "/" + subject + "/"){
                ...
                chapter = document.querySelector("body > header.interior-header > div > div.interior-header__title > div");
                if(chapter){
    ...
    if(path == "/"){
        ...
        if(title){
            ...
            presenceData.state = title.textContent.replace(' | The SparkNotes Blog','');
        }
        else{
            presenceData.startTimestamp = browsingStamp;
        ...
        title = document.querySelector("body > header.titleHeader--howTo > div > h1");
        if(title){
            presenceData.startTimestamp = browsingStamp;
            presenceData.details = "Viewing:";
        ...
        title = (document.querySelector("#results-search-input") as HTMLInputElement).value;
        presenceData.startTimestamp = browsingStamp;
        ...
        titlePage = document.querySelector("body > header.interior-header > div > div.interior-header__title > div > span.interior-header__title__pagetitle");
        if(chapter){
            presenceData.state = chapter.textContent;
        }
        else if(titlePage){
    ...
    else if(title == null && document.querySelector("body > header.TitleHeader_header.TitleHeader_header--noFear > div > div > h1")){
        title = document.querySelector("body > header.TitleHeader_header.TitleHeader_header--noFear > div > div > h1");
        presenceData.startTimestamp = browsingStamp;
        presenceData.details = "Viewing: " + title.textContent;
        titlePage = document.querySelector("body > header.interior-header > div > div.interior-header__title > div");
        if(titlePage){
            presenceData.state = titlePage.textContent;
        }
    }
