...
presence.on("UpdateData", async () => {
  ...
  if (path.includes("/episodes/")){
    const EpAndSeason = SouthParkData.children[0].props.title.text.split(' - ')[1].match(/([1-9]?[0-9]?[0-9])/g),
    EpTitle = SouthParkData.children[0].props.title.text.split(' - ')[2],
    title = SouthParkData.children[0].props.title.text.split(' - ')[0],
    timestamps = presence.getTimestamps(
      presence.timestampFromFormat(document.querySelector("div.edge-gui-current-time")?.textContent),
      presence.timestampFromFormat(document.querySelector("div.edge-gui-duration")?.textContent));
    
    if (video){
      presenceData.details = title;
      presenceData.state = `S${EpAndSeason[0]}:E${EpAndSeason[1]} ${EpTitle}`;

      presenceData.smallImageKey = (video.paused || isNaN(video.duration)) ? "pause" : "play";
      presenceData.smallImageText = (video.paused || isNaN(video.duration)) ? "Paused" : "Playing";
      ...
      presenceData.buttons = [
        {
          label: "Watch Episode",
          url: `https://www.southparkstudios.com/episodes/${document.location.pathname.split("/")[2]}`
        }
      ];
    ...
    const season = document.URL.match(/(season-[1-9]?[0-9])/)[0].replace("season-", "");
    ...
    EpAndSeason = document.querySelector("div > div.sub-header > span").textContent.match(/([1-9]?[0-9]?[0-9])/g),
    ...
    if (video){
      ...
      presenceData.buttons = [
        {
          label: "Watch Episode",
          url: `https://www.southparkstudios.com/collections/${document.location.pathname.split("/")[2]}`
        }
      ];
  ...
  pages: {
    ...
    "/news/": {
      ...
      buttons: [{
        label: "Read article",
        url: `https://www.southparkstudios.com/news/${document.location.pathname.split("/")[2]}`
      }]
    },
    "/avatar": {
      details: "Viewing page:",
