...
presence.on("UpdateData", async function () {
  ...
  if(!urlpath[urlpNum + 1])
    ...
    if(urlpath[num] === 'new')
      ...
      if(urlpath[urlpNum + 3]) presenceData.state = document.querySelector("li.b-breadcrumb-item>span.b-breadcrumb-text").textContent;
  ...
  else if (urlpath[urlpNum + 1] === "login")
  ...
  else if (urlpath[urlpNum + 1] === "content")
    presenceData.details = "Other";

  let smallimage = document.location.hostname.replace("www.snipes.", "").replace("www.snipesusa.", "");
