...
presence.on('UpdateData', async () => {
  const presenceData: PresenceData = {
    ...
    video: HTMLVideoElement = document.querySelector('#storyfire-player_html5_api');
  ...
  if (document.location.pathname.startsWith('/video-details')) {
    const timestamps = getTimestamps(
      ...
      videoTitle = document.querySelector('.title > div:not(.series)').textContent;
    ...
    if (await presence.getSetting('showsearchterm') && document.querySelector('.content-header > span'))
      presenceData.state = document.querySelector('.content-header > span').childNodes[1].textContent;
  }
