		...
		if (document.location.href.includes("/p/")) {
			postGQLAPI(
				...
				if (res.data.comments[0].content[0].__typename === "Image") {
					...
					presenceData.buttons = [{ url: document.location.href, label: "View Post" }];
			...
			postGQLAPI(
				...
				presenceData.buttons = [{ url: document.location.href, label: "View Clan" }];
				presence.setActivity(presenceData);
			});
			...
			presenceData.buttons = [{ url: document.location.href, label: "View User" }];
			presence.setActivity(presenceData);
