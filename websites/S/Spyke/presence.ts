...
async function postGQLAPI(
  ...
  const token = JSON.parse(
      JSON.parse(localStorage.getItem("persist:Token_Data_Persist")).jwtIdToken
    ),
    response = await fetch("https://api.spyke.social/graphql", {
      method: "POST",
      headers: {
...
presence.on("UpdateData", () => {
  ...
  if (nurl !== url) {
    ...
    presenceData.buttons = [
      { label: "View Spyke", url: "https://spyke.social/" },
    ];
