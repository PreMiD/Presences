...
presence.on("UpdateData", async () => {
	...
	if (url.includes("/intensivao") && !url.split("/intensivao/")[1]) presenceData.details = "Procurando aulas no Intensivão";
	...
	if (subject && title) {
		...
		presenceData.state = document.querySelector(".c-sidebar__item.is-active .c-sidebar__item-txt")?.textContent;
	} else {
		...
		if (subject && title) {
			...
			const player: HTMLVideoElement = document.querySelector(".fp-player video");
			...
			if (player) {
				...
				if (!player.paused)
					presenceData.endTimestamp = presence.getTimestampsfromMedia(player)[1];
			}
		}
		...
		if (url.includes("/videos/")) {
			...
			if (title) {
				...
				presenceData.state = document.querySelector(".areaTabs .active .areaStoodi")?.textContent;
			}
		}
