...
presence.on('UpdateData', async() => {
    ...
    const tempo = Math.floor(Date.now() / 1000),
        ...
        comentario: HTMLElement = document.querySelector("#corpo > div.conteudoBox.commentBoxAncora > div.commentMain > div.js_comment_view_box"),
        ...
        lancamentosCategorizar: HTMLElement = document.querySelector("#corpo > div:nth-child(2) > div.test > div > a.buttonLink.active"),
        topCategorizar: HTMLElement = document.querySelector("#corpo > div.conteudoBox > div:nth-child(5) > a.buttonLink.active"),
        selectHunter: HTMLSelectElement = document.querySelector("#corpo > div:nth-child(1) > div.box_content > div > select"),
        indicacaoTipo: HTMLSelectElement = document.querySelector("#conteudoSelect"),
        indicacaoGenero: HTMLSelectElement = document.querySelector("#generoSelect"),
        nomePerfil: HTMLElement = document.querySelector("#corpo > header > div.perfil_header_photos > div.perfil_box_photo > h1"),
        descricaoPerfil: HTMLElement = document.querySelector("#menu-link-perfil-sobre > ul:nth-child(2) > li.sizeFull"),
        nomeidentificadorPerfilAmigos: HTMLElement = document.querySelector("#corpo > div.conteudoBoxHome > h2"),
        nomeidentificadorPerfil: HTMLElement = document.querySelector("#corpo > div.conteudoBox > h1");
    ...
    if (path == '/') {
        ...
        if (titulo != 'Lista de Gêneros') {
            presenceData.details =  'Gênero: '+ titulo.slice(titulo.search("Genero") + 6);
        ...
        if (nome != null && nome.textContent.includes('Lista de Todos os Marcados')) {
            ...
            presenceData.state = descricaoPerfil.innerText.replace('❞', '').replace('❝', '');
        } else if (nome != null && nome.textContent.includes('Lista de Favorito')) {
            presenceData.state = 'Lista de favoritos';
        } else if (nome != null && nome.textContent.includes('Lista de Ver Depois')) {
            presenceData.state = 'Lista de assistir depois';
        } else if (nome != null && nome.textContent.includes('Lista de Arquivado / Abandonado')) {
            presenceData.state = 'Lista de dropados';
        } else if (nome != null && nome.textContent.includes('Lista de Notificados')) {
        ...
        } else if (nomeidentificadorPerfil != null && nomeidentificadorPerfil.textContent.includes('Lista de Galerias')) {
            presenceData.state = 'Galeria';
        } else if (nomeidentificadorPerfil != null && nomeidentificadorPerfil.textContent.includes('Lista de Imagens')) {
        ...
        presenceData.state = 'Em ' + topCategorizar.textContent.slice(topCategorizar.textContent.search('Top') + 4);
        presenceData.startTimestamp = tempo;
    } else if (path.includes('/top')) {
        presenceData.details = 'Top Conteúdo';
        presenceData.state = 'Do(a) ' + topCategorizar.textContent.slice(topCategorizar.textContent.search('Top 100-') + 10);
        presenceData.startTimestamp = tempo;
        ...
        if (indicacaoTipo.value != '') {
            if (indicacaoGenero.value != '') {
                presenceData.state = indicacaoTipo[indicacaoTipo.selectedIndex].innerText + ' |' + indicacaoGenero[indicacaoGenero.selectedIndex].innerText;
            } else {
                presenceData.state = indicacaoTipo[indicacaoTipo.selectedIndex].innerText;
            }
        } else if (indicacaoGenero.value != '') {
            if (indicacaoTipo.value != '') {
                presenceData.state = indicacaoTipo[indicacaoTipo.selectedIndex].innerText + ' |' + indicacaoGenero[indicacaoGenero.selectedIndex].innerText;
            } else {
                presenceData.state = indicacaoGenero[indicacaoGenero.selectedIndex].innerText;
            }
        }
        presenceData.startTimestamp = tempo;
    } else if (path == '/chat') {
    ...
    if (infoDiv != null && infoDiv.style.display != 'none') {
        if (infoDivTitulo != null) {
            ...
            infoDivTitulo.textContent.includes('Você precisa esta logado para visualizar suas Notificações!') || 
            infoDivTitulo.textContent.includes('Você precisa esta logado para visualizar a lista de Próximos Vídeos a serem Assistidos!') || 
            infoDivTitulo.textContent.includes('Você precisa esta logado para visualizar a lista de Próximos Vídeos a serem Assistidos!')) {
                ...
                } else if (infoDivTitulo.textContent.includes('um conteúdo no histórico')) {
                    presenceData.state = 'Em menu do histórico';
                } else if (infoDivTitulo.textContent.includes('no histórico de Download')) {
