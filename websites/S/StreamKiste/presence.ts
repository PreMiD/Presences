interface PageContext {
  ...
  exec: (
    ...
    options?: any
  ) => Promise<PresenceData | null | undefined> | PresenceData | null | undefined
...
const pages: PageContext[] = [
  {
    ...
    exec: (context, data, { strings, images, frame }: ExecutionArguments) => {
      ...
      if (!partName)
        return null
      const type: 'movie' | 'series' | 'other' = partName === 'serie'
        ? 'series'
        : partName === 'movie'
          ? 'movie'
          : 'other'
...
presence.on('iFrameData', (data: unknown) => {
  frameData = data as VideoContext
})
