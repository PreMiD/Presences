...
presence.on("UpdateData", async () => {
    ...
    data.presence = {
      "/programs/([0-9]+)/play": {
        async setPresenceData(){
          ...
          data.meta["title"] = document.querySelector("h1.vjs-metadata.vjs-metadata--title")?.textContent ?? "Loading...";
          data.meta["episode"] = document.querySelector("h2.vjs-metadata.vjs-metadata--subtitle")?.textContent;
          ...
          presenceData.buttons = [{
            label: data.meta["episode"] ? (await strings).viewEpisode : "Watch Video",
            url: document.URL
          }];
    ...
    for (const setting of data.settings){
       const settingValue = await presence.getSetting(setting.id).catch(() => null);

       if ((!settingValue && !setting.value || settingValue === setting.value) && setting.delete && !setting.presence)
           ...
           if (document.location.pathname.match(presenceSetting.page)){
             if (presenceSetting.setTo && !presenceSetting.replace)
              presenceData[presenceSetting.uses as "details"] = presenceSetting.setTo;
                ...
                if (replaced) presenceData[presenceSetting.uses as "details"] = replaced;
             }
