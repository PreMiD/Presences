...
presence.on("UpdateData", async () => {
  ...
  presenceSettings: {
    [key in keyof PresenceData]: boolean | {
      condition: { 
        page: string,
        setTo?: string,
        enabled?: boolean,
  ...
  pages = {
    "/schedule"(){
      ...
      ).find(x => YouCanSeeThis(x))?.textContent;
    ...
    "/stream/"(){
      ...
      if (isNaN(duration)){
        ...
        presenceData.buttons = [
          ...
          {
            label: "View Series",
            url: document.querySelector<HTMLAnchorElement>("div.breadcrumb > span:nth-child(3) > a").href
          }
        ];
  ...
  for (const [setting, settingData] of Object.entries(presenceSettings)){
    if (typeof settingData !== "boolean"){
      for (const condition of settingData.condition){
        if (condition.page === PageStatus && condition.setTo && !condition.enabled && !condition.replace && !condition.delete){
          presenceData[setting as "state" | "details"] = condition.setTo;

        } else if (condition.page === PageStatus && !condition.enabled && condition.delete){
