...
presence.on("UpdateData", async () => {
	...
	if (document.location.pathname.startsWith("/song/")) {
		...
		let songArtist = document.querySelector(".song").children[2].innerHTML.split("&nbsp;")[0];
	...
	if (document.location.pathname === "/java") {
		if (document.location.href.includes("?")) {
			...
			const image = document.querySelector(".active")
						...
						.replace("<img class=\"disc\" src=\"/media/images/chart-", "")
						.replace(".jpg\">", "").toUpperCase();
			let imageKey = "";

			switch(image) {
				case "SEARCH": imageKey = Assets.CHART_SEARCH; presenceData.state = "Searching charts"; break;
				case "HYPORANKED": imageKey = Assets.CHART_HYPORANKED; break;
				case "EXPONENTIAL": imageKey = Assets.CHART_EXPONENTIAL; break;
				case "HARD": imageKey = Assets.CHART_HARD; break;
				case "INTERMEDIATE": imageKey = Assets.CHART_INTERMEDIATE; break;
				case "EASY": imageKey = Assets.CHART_EASY; break;
				case "HISTORY": imageKey = Assets.CHART_HISTORY; break;
				case "POSSESION": imageKey = Assets.CHART_POSSESION; break;
				case "HOT": imageKey = Assets.CHART_HOT; break;
				case "COLD": imageKey = Assets.CHART_COLD; break;
				case "MULTIPLAYER": imageKey = Assets.CHART_MULTIPLAYER; presenceData.state = "Selecting multiplayer room"; break;
			}
