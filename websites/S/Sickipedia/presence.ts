...
presence.on("UpdateData", async () => {
  ...
  else if(matchPage("/message"))
    updateData("Viewing page:", "Messages");
  ...
  else if(matchPage("/account/login"))
    updateData("Viewing page:", "Login");
  ...
  if(matchPage(null, "/user/"))
    ...
    updateData("Viewing a tag (Jokes):", window.location.pathname.split("/")[3]);
  else if(matchPage(null, "/pics/tag/"))
    updateData("Viewing a tag (Images):", window.location.pathname.split("/")[3]);
  else if(matchPage(null, "/joke/category/"))
    updateData("Viewing a category:", `ID: ${window.location.pathname.split("/")[3]}`);
  else if(matchPage(null, "/joke/subcategory/"))
    updateData("Viewing a subcategory:", `ID: ${window.location.pathname.split("/")[3]}`);
  else if(matchPage(null, "/joke/"))
    updateData("Viewing a joke:", `ID: ${window.location.pathname.split("/")[2]}`);
  else if(matchPage(null, "/pics/"))
    updateData("Viewing a picture:", `ID: ${window.location.pathname.split("/")[2]}`);
  presence.setActivity(data.presenceData);
});

function matchPage(href: string, option?: string) {
  if(window.location.pathname === href || (option && window.location.pathname.startsWith(option) && window.location.pathname.split("/")[2])) 
    return true;
  else
    return false;
}
