...
presence.on("UpdateData", () => {
  ...
  if (page.toLowerCase() === "/") {
    ...
    presenceData.state =
      window.location.search.slice(8) + " " + "numaralı kodu düzenliyor!";
    presenceData.startTimestamp = browsingStamp;
    ...
    presenceData.state =
      document.querySelector("body > center > div > a:nth-child(6)")
        .textContent +
      " " +
    ...
    presenceData.state =
      document.querySelector(
        "body > div.jumbotron.jumbotron-x > div > center > h2"
      ).textContent +
      " " +
