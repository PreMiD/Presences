...
presence.on("UpdateData", async function () {
  ...
  if(document.location.host === 'www.wizardingworld.com') {
    if(!urlpath[1])
      ...
      if(urlpath[2]) {
        presenceData.state = document.querySelector("h1.ArticleHero_title__cOam6")?.textContent || "Unknown";
      ...
      if(urlpath[2]) {
        const post = document.querySelector("h1.ArticleHero_title__cOam6"),
            label = (post?.textContent.length >= 15) ? post?.textContent.substring(0, 15) + "..." : post?.textContent;
      ...
      if(urlpath[2] === 'books' || urlpath[2] === 'films' || urlpath[2] === 'portkey-games' || urlpath[2] === 'on-stage' || urlpath[2] === 'experiences') {
        if(urlpath[3]) {
          let ctopic = "Loading...";

          presenceData.state = document.querySelector("h1.Header_productName__8oV2G")?.textContent || "Unknown";
          ...
          presenceData.state = document.querySelector("h1.DiscoverListHeader_header__3ivqr")?.textContent || "Unknown";
      }
    } else if(urlpath[1] === 'collections') {
      presenceData.details = 'Collections';

      if(urlpath[2]) {
        presenceData.state = document.querySelector("h1.CollectionHero_header__3rDGu")?.textContent || "Unknown";
    ...
    if(document.querySelector("span.HogwartsHouse_houseName__CykI1")) {
      presenceData.smallImageKey = document.querySelector("span.HogwartsHouse_houseName__CykI1")?.textContent.toLocaleLowerCase();
      presenceData.smallImageText = document.querySelector("span.HogwartsHouse_houseName__CykI1")?.textContent;
    }
  }
