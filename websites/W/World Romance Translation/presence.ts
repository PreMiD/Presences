...
presence.on('UpdateData', async () => {
  const presenceData: PresenceData = {
    largeImageKey: 'https://cdn.rcd.gg/PreMiD/websites/W/World%20Romance%20Translation/assets/logo.png',
    startTimestamp: browsingTimestamp,
  }
  const { pathname, href } = document.location

  if (pathname === '/') {
    const searchQuery = document.querySelector<HTMLHeadingElement>('div.releases > h1')
    if (!searchQuery) {
      presenceData.details = 'Melihat Homepage'
    }
    else {
    ...
    const mangaName = document.querySelector<HTMLHeadingElement>('h1.entry-title')
    const chapterNumber = document.querySelector<HTMLSelectElement>('select#chapter')
    const pageNumber = document.querySelector<HTMLSelectElement>(
      'select#select-paged',
    )
    ...
    const mangaName = document.querySelector<HTMLHeadingElement>('h1.entry-title')
    presenceData.details = pathname === '/manga/' ? 'Melihat Daftar Manga' : 'Melihat Detail Manga'
    if (mangaName)
