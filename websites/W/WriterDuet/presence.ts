...
presence.on("UpdateData", async () => {
	...
	switch(page.split("#")[0]) {
		...
		case Pages.article: {
			...
			presenceData.state = document.querySelector("#categoryHead > h1")?.textContent
								|| document.querySelector(".title").textContent;
		}
		break;

		case Pages.pricing: presenceData.details = "Reviewing Payement Plans";
		break;

		case Pages.blog: {
			if (subpage) {
				presenceData.details = "Reading Blog Article";
				presenceData.state = document.querySelector(".jupiterx-post-title").textContent;
