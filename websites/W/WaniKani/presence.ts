...
presence.on("UpdateData", () => {
  ...
  switch (const presence: Presence = new Presence({
    clientId: "800166344023867443"
  }),
  largeImageKey = "logo";
...
presence.on("UpdateData", () => {
    ...
    case "www.wanikani.com": {
      switch (pathname) {
        ...
        case "/login": {
          ...
          if (buttons.length === 2) {
            const lessons: number = +buttons[0].querySelector("span").textContent,
        ...
        case "/review/session": {
          const available: number = +(
            ...
            characterText: string = characterElement.textContent,
  ...
  if (typeof details !== "undefined") presenceData.details = details;
    ...
    case "www.wanikani.com": {
      switch (pathname) {
        ...
        case "/review/session": {
          ...
          smallImageText = `${completed} complete, ${+(
              document.querySelector("#available-count") as HTMLElement
            ).textContent} remaining. (${correctRate}%)`;
          smallImageKey = characterType;
          if (eventType !== 2) {
            elapsed = Math.round(Date.now() / 1000);
            eventType = 2;
          ...
          try {
            const 
              ...
              characterType: string =
                document.querySelector("#main-info").className;
            details = "Learning Lessons";
            state = `${document.querySelector("#character").textContent} - ${characterMeaning}`;
            smallImageKey = characterType;
            smallImageText = `${totalStats[0].textContent} radicals | ${totalStats[1].textContent} kanji | ${totalStats[2].textContent} vocab | ${completed} complete`;
            if (eventType !== 2) {
              elapsed = Math.round(Date.now() / 1000);
            ...
            const 
              ...
              totalStats: NodeList =
                document.querySelectorAll("#stats li > span");
            details = "Practicing Lessons";
            state = `${document.querySelector("#character").textContent} | ${capitalize(
              characterType
            )} ${capitalize(questionType)}`;
            smallImageKey = characterType;
            smallImageText = `${totalStats[0].textContent} radicals | ${totalStats[1].textContent} kanji | ${totalStats[2].textContent} vocab | ${completed} complete`;
