...
presence.on("UpdateData", async () => {
  ...
  presenceData.details = await presence.getSetting("homepageMessage");
    ...
    presenceData.details = await presence.getSetting("testMessage");
      ...
      const memberCount = document.querySelector("accordion-section:nth-child(2) span.toggle-number")?.textContent.match(/([0-9]+)/);

      if (document.querySelector("#video-button")){
        presenceData.details = "In a meeting";
        presenceData.state = memberCount ? `${memberCount[0]} ${memberCount[0] === "1" ? "participant" : "participants"} in a room` : "Unknown participant(s) in a room";

        presenceData.smallImageKey = document.querySelector("#video-button").getAttribute("aria-label").endsWith("off") ? "vcall" : "call";
      } else
