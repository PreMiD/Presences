const presence = new Presence({
  ...
  presence.on("UpdateData", () => {
    ...
    if (document.querySelector("#search-story") && (document.querySelector("#search-story") as HTMLInputElement).value !== "") {
      data.details = "Searching:";
      data.state = (document.querySelector("#search-story") as HTMLInputElement).value;
      ...
      if (/^\/$/.test(pathname)) data.details = "Viewing Home Page";
        ...
        data.state = "📋 " + manga.charAt(0).toUpperCase() + manga.slice(1).toLowerCase() + " manga";
      }
      ...
      } else if (/^\/(manga+)-[a-z0-9]+\/(chapter+)-[a-z0-9]+?$/i.test(pathname)) {
          
        const comicLink = (document.querySelector(".panel-breadcrumb a:nth-child(3)") as HTMLAnchorElement).href,
        chapter = document.querySelector(".panel-breadcrumb a:nth-child(5)").textContent;
        ...
        data.details = document.querySelector(".panel-breadcrumb a:nth-child(3)").textContent;
