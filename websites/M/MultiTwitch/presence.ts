...
let currentStreams = document.location.pathname.split("/").filter(function(str) {
	return str !== "";
});
if(currentStreams.length === 0) 
	presenceData.details = "On the home screen";
 else 
	presenceData.details = `Watching ${currentStreams.length} ${currentStreams.length > 1 ? "streams" : "stream"}`;
