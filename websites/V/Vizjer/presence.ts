...
presence.on('UpdateData', async () => {
  ...
  const series: string | null = pathname.startsWith('/series/index/') ? pathname : null
  const movies: string | null = pathname.startsWith('/filmy-online/') ? pathname : null
  switch (pathname) {
    case '/wyszukiwanie':
      if (search) {
    ...
    case movies:
      ...
      if (movies && movies.includes('sort:date')) {
        presenceData.details = 'Przegląda ostatnio zaaktualizowane filmy online...'
      }
      if (movies && movies.includes('sort:newlink'))
        presenceData.details = 'Przegląda ostatnio dodane filmy...'
      if (movies && movies.includes('sort:vote'))
        presenceData.details = 'Przegląda filmy z największą liczbą głosów...'
      if (movies && movies.includes('sort:premiere'))
        presenceData.details = 'Przegląda ostatnie premiery filmów...'
      if (movies && movies.includes('sort:view'))
        presenceData.details = 'Przegląda filmy z największą liczbą odsłon...'
      if (movies && movies.includes('sort:rate'))
    ...
    case series:
      ...
      if (series && (series.includes('sort:newest') || series.includes('sort:date')))
        presenceData.details = 'Przegląda najnowsze seriale online...'
      if (series && series.includes('sort:year'))
        presenceData.details = 'Przegląda ostatnie premiery seriali...'
      if (series && series.includes('sort:vote')) {
        presenceData.details = 'Przegląda seriale z największą liczbą głosów...'
      }
      if (series && series.includes('sort:view')) {
        presenceData.details = 'Przegląda seriale z największą liczbą odsłon...'
      }
      if (series && series.includes('sort:rate'))
