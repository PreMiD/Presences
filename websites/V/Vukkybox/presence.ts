const presence = new Presence({
  ...
  presence.on("UpdateData", async () => {
	...
	if(document.location.pathname == "/gallery"){
		...
		presenceData.state = `Unlocked ${document.getElementById("total_text").childNodes[3].textContent.substring(14,21)} Vukkies!`
		presenceData.startTimestamp = Date.now()
	} else if(document.location.pathname == "/"){
		presenceData.details = "Browsing the Homepage"
		presenceData.state = `Obtained ${document.getElementById("balance").innerText}!`
		presenceData.startTimestamp = Date.now()
	}else if(document.location.pathname == "/store"){
		presenceData.details = "Browsing the Store"
		presenceData.state = `${document.getElementById("balance").innerText} to spend!`
		presenceData.startTimestamp = Date.now()
	} else if(document.location.pathname == "/buyBox/classic"){
		presenceData.details = "Opening a Classic Box"
		presenceData.state = `${document.getElementById("balance").innerText} left to spend!`
		...
		if(document.getElementsByTagName("h1")[0].innerText.includes("sussy")){
			...
			presenceData.state = `Yay, its a valid coupon!`
	}
}
