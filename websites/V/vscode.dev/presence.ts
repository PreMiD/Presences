...
const KNOWN_EXTENSIONS: { [key: string]: { image: string } } = {
  ...
  '/webpack(\\.dev|\\.development|\\.prod|\\.production)?\\.config(\\.babel)?\\.(js|jsx|coffee|ts|json|json5|yaml|yml)/i': { image: Assets.Webpack },
  ...
  '/[\\/\\\\](?:magic[\\/\\\\]Magdir|file[\\/\\\\]magic)[\\/\\\\][-.\\w]+$/i': { image: Assets.Manifest },
  '/(\\\\|\\/)dev[-\\w]+\\1(?:[^\\\\\\/]+\\1)*(?!DESC|NOTES)(?:[A-Z][-A-Z]*)(?:\\.in)?$/': { image: Assets.Manifest },
  'lib/icons/.icondb.js': { image: Assets.Manifest },
  '/\\.git[\\/\\\\](.*[\\/\\\\])?(HEAD|ORIG_HEAD|packed-refs|logs[\\/\\\\](.+[\\/\\\\])?[^\\/\\\\]+)$/': { image: Assets.Manifest },
  ...
  '/\\.(rb|ru|ruby|erb|gemspec|god|mspec|pluginspec|podspec|rabl|rake|opal)$/i': { image: Assets.Ruby },
  '/^\\.?(irbrc|gemrc|pryrc|ruby-(gemset|version))$/i': {
    image: Assets.Ruby,
  },
  '/^(Appraisals|(Rake|[bB]uild|Cap|Danger|Deliver|Fast|Guard|Jar|Maven|Pod|Puppet|Snap)file(\\.lock)?)$/': { image: Assets.Ruby },
  ...
  '/^(\\.?bash(rc|[-_]?(profile|login|logout|history|prompt))|_osc|config|install-sh|PKGBUILD)$/i': { image: Assets.Shell },
  '/\\.(ksh|mksh|pdksh)$/i': { image: Assets.Shell },
  '.sh-session': { image: Assets.Shell },
  '/\\.zsh(-theme|_history)?$|^\\.?(antigen|zpreztorc|zlogin|zlogout|zprofile|zshenv|zshrc)$/i': { image: Assets.Shell },
  '/\\.fish$|^\\.fishrc$/i': { image: Assets.Shell },
  '/^\\.?(login|profile)$/': { image: Assets.Shell },
  '.inputrc': { image: Assets.Shell },
  '.tmux': { image: Assets.Shell },
  '/^(configure|config\\.(guess|rpath|status|sub)|depcomp|libtool|compile)$/': { image: Assets.Shell },
