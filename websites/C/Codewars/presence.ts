...
presence.on("UpdateData", async () => {
  ...
  if (pages[0] === 'dashboard') {
      ...
      data.state = Array.from(document.querySelectorAll('.icon-list > li > .is-active')).length + ' Languages selected';
  }
  else if (pages[0] === 'users' && pages[1] !== 'leaderboard' && pages[1] !== 'edit') {
