...
presence.on("UpdateData", async () => {
  ...
  if (!Routes[0] || Routes[0] === 'hot') {
    ...
    if (!Routes[1]) {
      ...
      data.state = 'Creating by ' + document.querySelector('.story-form__stamp .story-form__channels-dropdown button span').textContent;
    ...
    const user = document.querySelector('.sc-jAaTju.eNyegn a .sc-hSdWYo.eioDHx .sc-eHgmQL.hoWqal');
    ...
    data.state = document.querySelector('.weekly__menu .page-menu__item.-active').textContent;
    ...
    if (Routes[1]) {
      data.details = document.querySelector('.coub__description .description__info h5.description__title').textContent;
      data.state = document.querySelector('.coub__description .description__info .description__stamp').textContent;
    }
    ...
    // Search page
    data.details = 'Community: ' + document.querySelector('.hot__community .title h2').textContent;

    if (Routes[1] === 'featured') {
      // Featured community
      data.state = document.querySelector('.featured__menu .-active.page-menu__item').textContent + ' coubs';
    ...
    data.details = 'Channel: ' + document.querySelector('.channel .channel__description h1').textContent;
    
    if (Routes[1] === 'edit') {
      data.state = 'Edit channel';

    } else {
      data.state = document.querySelector('.profile__menu .page-menu__item.-active').textContent;
