...
presence.on("UpdateData", async function () {
  ...
  if(setting.timeElapsed && !setting.privacy) presenceData.startTimestamp = browsingStamp;

  if(!urlpath[1])
    presenceData.details = "Home";
  else if(urlpath[1] === 'world' || urlpath[1] === 'us' || urlpath[1] === 'politics' || urlpath[1] === 'business' || urlpath[1] === 'health' || urlpath[1] === 'entertainment' || urlpath[1] === 'style' || urlpath[1] === 'travel' || urlpath[1] === 'sport' || (urlpath[1] === 'sport' && document.querySelector("a.sc-fjdhpX.sc-chPdSV.ewmEKt")) || urlpath[1] === 'videos' || urlpath[1] === 'africa' || urlpath[1] === 'americas' || urlpath[1] === 'asia' || urlpath[1] === 'australia' || urlpath[1] === 'china' || urlpath[1] === 'europe' || urlpath[1] === 'india' || urlpath[1] === 'middle-east' || urlpath[1] === 'uk') {
    const category = document.querySelector("h1.metadata-header__title")?.textContent || document.querySelector(".Text-sc-1amvtpj-0-span.jKFEoX")?.textContent.replace("Follow CNN ", "") || "Not found";
  ...
  } else if((urlpath[1] >= '1900' && urlpath[1] <= new Date().getFullYear()) && (urlpath[2] >= '01' && urlpath[2] <= '12') && (urlpath[3] >= '01' && urlpath[3] <= '31')) {
    const article = document.querySelector("h1.pg-headline")?.textContent || "Not found";
    ...
    const profile = document.querySelector("div.cd__profile-name")?.textContent || "Not found";
    ...
    if(urlpath[2]) {
      if(urlpath[3] && urlpath[2] !== 'us') {
        const special = document.querySelector("h1.pg-headline")?.textContent || document.querySelector("li>a.geyBat")?.textContent || "Not found";

        presenceData.details = (setting.privacy) ? "Special" : "Special:";
        if(!setting.privacy) presenceData.state = special;
      } else {
        const special = document.querySelector("a.sc-fjdhpX.sc-chPdSV.ewmEKt")?.textContent || "Not found";
