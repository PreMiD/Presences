...
presence.on('UpdateData', async () => {
  const presenceData: PresenceData = {
    largeImageKey: 'https://cdn.rcd.gg/PreMiD/websites/C/Chia%20S%E1%BA%BB%20Nh%E1%BA%A1c/assets/logo.png',
  ...
  if (pathname.startsWith('/hd')) {
    if (pathname === '/hd/video.html') {
      presenceData.details = 'Đang tìm video'
    }
      ...
      const { paused, currentTime, duration } = document.querySelector<HTMLVideoElement>('video')!
      ...
      if (timestamps) {
        ...
        if (!paused) {
          [presenceData.startTimestamp, presenceData.endTimestamp] = presence.getTimestamps(duration, currentTime)
        }
      }
      if (buttons)
