...
presence.on("UpdateData", async () => {
    ...
    if (path == '/') {
        ...
        paginaAtual = document.querySelector("body > main > div > div:nth-child(1) > div > div.row > div > ul > li.active > span");
        ultimaPagina = document.querySelector("body > main > div > div:nth-child(1) > div > div.row > div > ul > li:nth-child(12) > a");
        presenceData.details = 'Últimas Atualizações';
        presenceData.state = 'Página ' + paginaAtual.innerText + ' de ' + ultimaPagina.innerText;
    } else if (path.includes('/manga/')) {
        (document.location.pathname.split('/').length - 1) == 4 
        ? (
                presenceData.details = nomeObraLeitor.innerText.replace('Manga', ' '), 
                presenceData.state = leitorCapitulo.innerText.slice(0, leitorCapitulo.innerText.search(':') + 1).replace(':', '')//,
            ) : (
