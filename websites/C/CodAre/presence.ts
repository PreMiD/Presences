...
presence.on("UpdateData", () => {
  ...
  if (page.toLowerCase() === "/") {
    ...
    presenceData.state = document.querySelector("body > div > center > div > div > div.topSectionNormal-2-vo2m > header > div.headerInfo-30uryT > div.nameTag-2IFDfL.userSelectText-1o1dQ7.nameTag-m8r81H > span").textContent + " " +"adlı kullanıcının profilini görüntülüyor!";
    presenceData.startTimestamp = browsingStamp;
  } else if (page.startsWith("/kod")) {
    presenceData.details = "Bir kod görüntülüyor:";
    presenceData.state = document.querySelector("#\\32  > div > div > div > div > h3").textContent + " " + "adlı kodu görüntülüyor!";
    presenceData.startTimestamp = browsingStamp;
  } 
