  ...
  const presenceData: PresenceData = {
    ...
    if(
      ...
      if(!document.location.pathname.includes("/episodio"))
        ...
        presenceData.details = titulo.replace(document.querySelector(`h1.Title > span`).textContent, ``).replace(/\s+/g, ` `).trim();
        presenceData.state = document.querySelector(`h1.Title > span`).textContent + " " + subtitulo;
      }
