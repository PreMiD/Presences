...
presence.on("UpdateData", async () => {
  const presenceData: PresenceData = {largeImageKey: "lslogo", smallImageKey: "lsminilogo", smallImageText: "leosight.cz"},
  path = document.location.pathname;
  
  //filtrace fóra//

  if(path == "/" && !window.location.href.includes("artic") && !window.location.href.includes("ctf") && !window.location.href.includes("eco") && !window.location.href.includes("guard")){
    ...
    item2 = document.querySelector("div.col-md-3.pull-left span.authorname") as HTMLElement
  ...
  else if(path.includes("/forum/") && !path.includes("new") && !path.includes("topic")){
  ...
  else if(path.includes("/forum/topic"))
  {
    ...
    item2 = document.querySelector("div.forum-message div.left a.pname") as HTMLElement;
    presenceData.details = item.innerText;
    ...
    if(path.includes("postavy"))
      ...
      presenceData.details = "Seznam frakcí"
        if(path.includes("/frakce/")){
          item = document.querySelectorAll("div.obsah.text-center h2")[1] as HTMLElement
