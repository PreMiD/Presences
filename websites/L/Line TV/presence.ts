...
presence.on("UpdateData", async () => {
    ...
    if (pathname.includes('/v/') || pathname.includes("/special/")){
        ...
        videoD: VideoDType = {
            ...
            isHighligh: (document.querySelector('tooltip') as HTMLElement)?.title.toLowerCase().includes("highlight") || videoData.sClipTitle.toLowerCase().includes("highlight"),
        ...
        if (video){ 
            ...
            else if (videoD.isHighligh && !videoD.episodes) videoD.genre = "HIGHLIGHT";
            else if (videoD.isHighligh && videoD.episodes) videoD.genre = "HIGHLIGHT_";
            else if (videoData.sLiveStatus) videoD.genre = "LIVE";
            else if (videoD.isLowerThan1_5mins && !videoD.isHighligh && !videoD.isTrailer && video.currentTime && videoD.episodes) videoD.genre = "PREVIEW";

            videoD.title = pDetail.replace("%title%", videoD.title)
            ...
            switch (videoD.genre) {
                ...
                case "ARTIST":
                    presenceData.details = videoD.title;
                    presenceData.state = `Music ${videoD.episodes ? ` | ${(await strings).episode}${videoD.epText}` : ""}`;
                    videoD.buttonLabel = videoD.epText ? (await strings).viewEpisode : (await strings).viewSong;
                    break;

                case "ENTERTAINMENT":
                    presenceData.details = videoD.title;
                    presenceData.state = videoD.epText ? `${(await strings).episode} ${videoD.epText}` : "Entertainment";
                    videoD.buttonLabel = videoD.epText ? videoD.buttonLabel : (await strings).viewVideo;
                    break;

                case "LIFE":
                    presenceData.details = videoD.title;
                    presenceData.state = `${videoD.episodes ? `${(await strings).episode} ${videoD.epText}`: "Life style"}`;
                    videoD.buttonLabel = videoD.episodes ? (await strings).viewEpisode : (await strings).viewVideo;
                    break;
                ...
                case "HIGHLIGHT_":
                    presenceData.details = videoD.title;
                    presenceData.state = `${(await strings).episode}${videoD.epText} | Highlight`;
                    videoD.buttonLabel = (await strings).viewVideo;
                    break;
                ...
                case "MOVIE":
                    presenceData.details = videoD.title;
                    presenceData.state = `${videoD.episodes ? `Movie Clip ${(videoD.epText.startsWith(" [") || videoD.epText.startsWith(" (")) ? `|${videoD.epText}` : `| ${(await strings).episode}${videoD.epText}`}`: ""}`;
                    videoD.buttonLabel =  (await strings).viewVideo;
                    break;

                case "LIVE":
                    presenceData.details = videoD.title;
                    presenceData.state = `Live ${videoD.episodes ? ` | ${(await strings).episode}${videoD.epText}` : ""}`;
            ...
            presenceData.smallImageKey = video.paused ? "pause" : videoD.smallImagePlay;
            presenceData.smallImageText = video.paused ? (await strings).paused : (await strings).play;
            ...
            if (showButton){
                presenceData.buttons = [{
                    ...
                    url: document.baseURI.length > 512 ? `https://tv.line.me/v/${videoD.id}` : document.baseURI
                }];
        ...
        presenceData.details = `${(await strings).searchFor} ${showSearch ? query : "(Hidden)"}`;
