...
presence.on("UpdateData", () => {
	...
	function getAdvertiserName(): string {
		...
		if (name)
			...
			// Pro seller
			let fallbackName = document.querySelector(".styles_wrapperBottom__unGZF > div:nth-child(2) > h2:nth-child(1)")?.textContent;
			if (!fallbackName)
				fallbackName = document.querySelector(".styles_wrapperBottom__unGZF > div:nth-child(2) > a:nth-child(1)")?.textContent;
	...
	if (document.location.pathname.includes("/deposer-une-annonce"))
	...
	else if (document.location.pathname.includes("/profil/") || document.location.pathname.includes("/profile/")) {
		presenceData.state = `Profil de ${document.querySelector("h3.mr-lg")?.textContent}`;
		...
		presenceData.state = `Annonces pour «${
			document.querySelector("h1")?.textContent.trim().split("«").pop().split("»")[0]
		}»`;
