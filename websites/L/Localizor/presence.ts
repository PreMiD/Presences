...
presence.on("UpdateData", async () => {
	...
	if (page === "/") presenceData.details = "Browsing home page";
	...
	} else if (page.includes("/leaderboard")) presenceData.details = "Viewing Leaderboard";
		...
		const gameImage = document.querySelector<HTMLImageElement>(
			"div.game-card__image"
		...
		presenceData.details = "Translating " + document.querySelector<HTMLHeadingElement>(
			"div.mx-md-2.m-0 a"
		).textContent;
		presenceData.state = "Key: " + document.querySelector<HTMLDivElement>(
		...
		presenceData.smallImageText = "Translating to" + document.querySelector<HTMLDivElement>(
	...
	if (!images && presenceData.largeImageKey) presenceData.largeImageKey = "logo";
