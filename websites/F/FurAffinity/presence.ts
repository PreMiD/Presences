...
function checkCurrentPage() {
    if (document.location.hostname == "www.furaffinity.net") {
        ...
        else if (document.location.pathname.includes("/msg/pms/") && showBrowsingNotes && showBrowsingAccount) {
            switch (document.location.hash) {
                case '#message': {
                    presenceData.details = "Reads a note";
                    presenceData.state = '"' + document.querySelector('#message h2').innerHTML + '"';
                    break;
                }
                case '#MsgForm': {
