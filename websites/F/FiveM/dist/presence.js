...
presence.on("UpdateData", () => {
    ...
    else if (hostname === "docs.fivem.net" || hostname === "www.docs.fivem.net") {
        presenceData.details = "Viewing documentation:";
        if (pathname.startsWith("/docs/")) {
            presenceData.state = document.querySelector("body > div > div > div > main > div > h1:nth-child(1)").textContent;
        }
    }
    else if (hostname === "servers.fivem.net" || hostname === "www.servers.fivem.net") {
        presenceData.details = "Viewing server:";
        if (pathname.startsWith("/servers/detail/")) {
            presenceData.state = document.querySelector("body > app-root > div > div.app-root > servers > app-servers-detail > div.info.one-info > h1").textContent;
        }
