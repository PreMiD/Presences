	...
	switch (document.location.hostname) {
			case "81dojo.com":
					...
					// todo: statuses for more pages
							...
							if (parseFloat(document.getElementById("layerLogin").style.zIndex) == 2)
									presenceData.details = "Logging in to client";
							else {
									let server = document
											.getElementById("header-serverName")
											.innerText.slice(0, -3);
									let username = document
											.getElementById("header-playerName")
											.innerText.slice(0, -3);
									if (document.getElementById("layerLobby").style.display == "block")
											presenceData.details = "In " + server + " lobby";
									else {
											let playerElements = document.querySelectorAll(
													"[id='player-info-name']"
											);
											...
											if (username in players) {
													...
													const opponentRate = playerElements[
															1 - playerElementIndex
													].parentNode.children
															.item(4)
															.innerHTML.slice(3, 7);
