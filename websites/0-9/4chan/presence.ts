...
presence.on("UpdateData", async () => {
	...
	if (document.location.pathname === "/") {
		...
		if (document.location.pathname.includes("/thread/")) {
			var threadNum = document.location.pathname.split("/").at(-1);
			var threadSubject = document.getElementById(`pi${threadNum}`).querySelector(".subject").textContent;

			if (threadSubject) {
				presenceData.state = `>>${threadNum} - "${threadSubject}"`
			} else {
				presenceData.state = `>>${threadNum}`;
			}
