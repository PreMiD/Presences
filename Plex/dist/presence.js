    ...
    if (document.location.hostname == "app.plex.tv") {
        if (document.querySelector("#plex > div:nth-child(7) > div > div > video") !== null || document.querySelector("#plex > div:nth-child(7) > div > div > audio") !== null) {
            ...
            if(title) {
                ...
                if(title.length > 1) {
                    aux = title[0].replace(/[ES]/g, "").split("·");
                    season = aux[0];
                    episode = aux[1];
                    presenceData.details = `${season.padStart(2, "0")}x${episode.padStart(2, "0")} - ${presenceData.details}`; // title[0].replace(/[ES·]/g, "-") + "-" + user.textContent;
                }
