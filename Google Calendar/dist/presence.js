...
presence.on("UpdateData", () => {
  ...
  if (document.location.pathname == "/") {
    ...
    if (document.location.pathname.startsWith("/calendar/r/day")) {
      ...
      (data.details = "Viewing the schedule of"), (data.state = "custom days");
      data.startTimestamp = browsingStamp;
      presence.setActivity(data);
    } else if (document.location.pathname.startsWith("/calendar/r/eventedit")) {
      (data.details = "Editing a event"), (data.startTimestamp = browsingStamp);
      presence.setActivity(data);
      ...
      (data.details = "Exporting or ixporting"), (data.state = "a calendar");
      data.startTimestamp = browsingStamp;
      presence.setActivity(data);
    } else if (
      document.location.pathname.startsWith("/calendar/r/settings/calendar")
    ) {
      (data.details = "In the settings of a"), (data.state = "calendar");
      data.startTimestamp = browsingStamp;
      presence.setActivity(data);
    } else if (
