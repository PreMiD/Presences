...
presence.on("UpdateData", async () => {
	...
	if (window.location.href.includes("gdbrowser.com")) {
		// Levels
		if (
			(!isNaN(window.location.pathname.toLowerCase().replace("/", "")) &&
				window.location.pathname.toLowerCase() !== "/") ||
			window.location.pathname.toLowerCase() === "/daily" ||
			window.location.pathname.toLowerCase() === "/weekly"
		) {
			let downloads = document.getElementsByClassName(
				"inline smaller spaced"
			)[0].innerHTML;
			let likes = document.getElementsByClassName("inline smaller spaced")[1]
				.innerHTML;
