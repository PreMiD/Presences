...
presence.on("UpdateData", async () => {
    const page = document.location.pathname,
        productName = document.querySelector("#sp-title") as HTMLElement || document.querySelector("#ProductTitle > div.h1-container > h1 > span.title") as HTMLElement,
        price = document.querySelector(".lastPrice") as HTMLElement || document.querySelector("#col-center > div > div.border-e3e3e3 > ul.table-ul.gray-content.border-top-e3e3e3 > li.row-li.product-detail-product-price-info-ab-variant > ul > li.col-li.gg-w-17.gg-d-17.gg-t-16.gg-m-16.posr > div:nth-child(2) > div > div.low-price.extra-price.all-price.robotobold.clear.pl8.pt5") as HTMLElement,
        seller = document.querySelector("#store-page-title > div.store-name > h1") as HTMLElement;
    ...
    if (productName && productName.textContent != "") {
    ...
    } else if (pages[page] || pages[page.slice(0, -1)]) { // We are using slice(0, -1) here to slice the last "/" and check if there's a record on pages variable.
        ...
        data.state = document.title && document.title.includes(" - GittiGidiyor") ? document.title.replace(" - GittiGidiyor", "") : "";
